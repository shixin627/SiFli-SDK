...
## 10. J-Link DSK 与 Open Flashloader
SDK中的 _tools/flash/jlink_drv_ 是思澈提供的J-Link Device Support Kit(DSK) 静态包。<br/>
目录根下的 _JLinkDevices.xml_ 用于描述设备、flash bank和loader关系，各子目录中的elf文件是对应的Open Flashloader algorithm。<br/>

使用该DSK包时，请安装 J-Link V7.62 或以上版本，并将 _tools/flash/jlink_drv_ 目录内容整体复制到当前用户的 _JLinkDevices\\SiFli_ 目录。<br/>
Windows 默认目录如下：<br/>
_C:\Users\\<USER>\\AppData\\Roaming\\SEGGER\\JLinkDevices\\SiFli_<br/>

```{note}
1. J-Link V7.62 起会递归扫描 _JLinkDevices_ 目录下的所有 `*.xml` 文件，因此不需要修改J-Link安装目录中的 _JLinkDevices.xml_ 。
2. XML中的loader路径相对 _JLinkDevices.xml_ 解析，因此必须整体复制 _tools/flash/jlink_drv_ 目录，不能只拷贝部分elf文件。
3. 选择设备时，需要与板级工程 _rtconfig.py_ 中的 _JLINK_DEVICE_ 保持一致，例如 _SF32LB56X_ 、_SF32LB56X_NAND_ 、_SF32LB52X_NOR_ 等。
4. 同一个 _FlashBankInfo_ 下可以挂多个 _LoaderInfo_ ，J-Link 默认选择列表中的第一个loader。如果需要显式选择其他loader，可以使用命令行参数
   `_device "<DeviceName>?BankAddr=<FlashBankBase>&Loader=<LoaderName>"_`，
   或在J-Link Script中调用
   `_JLINK_ExecCommand("DEVICE_SelectLoader BankAddr=<FlashBankBase> Loader=<LoaderName>");_`
5. 无后缀的设备名，如 _SF32LB52X_ 、_SF32LB55X_ 、_SF32LB56X_ 、_SF32LB58X_ ，现在已经包含完整的bank和loader信息，且默认优先使用NOR相关loader。带后缀的设备名目前仍然保留，用于兼容现有脚本。
```
