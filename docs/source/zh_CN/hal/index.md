...
```{only} SF32LB57X
SF32LB57x系列有三个CPU，HCPU用于高性能计算，ACPU可用于音频和图形处理，LCPU适用于执行功耗敏感场景的任务，用户可以将高性能处理运行在HCPU上，例如图形显示，而将低功耗的功能实现在LCPU上，例如BLE、传感器数据处理等，这样可以使得在处理低功耗任务的时候，HCPU被关闭，降低系统功耗。
```
