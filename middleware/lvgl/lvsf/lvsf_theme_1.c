...
#define DARK_COLOR_SCR         lv_color_black()
...
typedef struct
{
    ...
    lv_style_t comp_mos;        // Modular small
#endif
...
static void style_init(void)
{
    ...
    lv_style_set_border_width(&styles->card, 0);//BORDER_WIDTH);
    lv_style_set_border_post(&styles->card, true);
    lv_style_set_text_color(&styles->card, color_text);
    lv_style_set_pad_all(&styles->card, 0); //PAD_DEF);
...
#else
    lv_style_set_width(&styles->chart_series, lv_disp_dpx(theme.disp, 8));
    lv_style_set_height(&styles->chart_series, lv_disp_dpx(theme.disp, 8));
#endif
...
#else
    lv_style_set_width(&styles->chart_indic, lv_disp_dpx(theme.disp, 8));
    lv_style_set_height(&styles->chart_indic, lv_disp_dpx(theme.disp, 8));
#endif
...
#if LV_USE_METER&&defined(DISABLE_LVGL_V9)
...
#if LVSF_USE_HEADER
    style_init_reset(&styles->header);
    lv_style_set_bg_color(&styles->header, theme.color_primary);
    lv_style_set_bg_opa(&styles->header, LV_OPA_TRANSP);
#endif

#if LVSF_USE_COMPOSITE
    style_init_reset(&styles->comp_mos);
    lv_style_set_bg_opa(&styles->comp_mos, LV_OPA_TRANSP);
    lv_style_set_text_font(&styles->comp_mos, theme.font_small);
    lv_style_set_line_width(&styles->comp_mos, MODULAR_SMALL_LINE_WIDTH);
#endif

#if LVSF_USE_CORNER
    style_init_reset(&styles->corner);
    lv_style_set_bg_opa(&styles->corner, LV_OPA_TRANSP);
    lv_style_set_text_font(&styles->corner, theme.font_small);
#endif
...
bool lvsf_theme_1_is_inited(void)
{
    return  LV_GC_ROOT(_lv_theme_default_styles) == NULL ? false : true;
}

lv_theme_t *lvsf_theme_1_get(void)
{
    if (!lvsf_theme_1_is_inited())
    {
        return NULL;
    }
...
lv_theme_t *lvsf_theme_1_init(lv_disp_t *disp, lv_color_t color_primary, lv_color_t color_secondary, uint32_t flags,
                              const lv_font_t *font_small, const lv_font_t *font_normal, const lv_font_t *font_subtitle,
                              const lv_font_t *font_title, const lv_font_t *font_bigl)
{
    ...
    theme.font_bigl = font_bigl;
    theme.font_large = font_title;  // V8 added, assign for backward compatible
    theme.apply_cb = theme_apply;
    theme.flags = flags ? MODE_DARK : 0;
...
#endif
#if LVSF_USE_HEADER
    else if (lv_obj_check_type(obj, &lv_lvsfheader_class))
    {
        lv_obj_add_style(obj, &styles->header, 0);
    }
#endif
#if LVSF_USE_COMPOSITE
    else if (lv_obj_check_type(obj, &lv_lvsfcomp_class))
    {
        lv_obj_add_style(obj, &styles->comp_mos, 0);
    }
#endif
#if LVSF_USE_CORNER
    else if (lv_obj_check_type(obj, &lv_lvsfcorner_class))
    {
        lv_obj_add_style(obj, &styles->corner, 0);
    }
#endif
...
void lv_theme_1_init()
{
    ...
    lv_theme_t *th;
    if (lvsf_theme_1_is_inited() == false)
    {
        th = lvsf_theme_1_init(disp, lv_palette_main(LV_PALETTE_BLUE), lv_palette_main(LV_PALETTE_RED),
                               LV_THEME_DEFAULT_DARK,
                               LV_THEME_DEFAULT_FONT_SMALL, LV_THEME_DEFAULT_FONT_NORMAL, LV_THEME_DEFAULT_FONT_SUBTITLE, LV_THEME_DEFAULT_FONT_TITLE, LV_THEME_DEFAULT_FONT_BIGL);
    }
    else
    {
        th = lvsf_theme_1_get();
