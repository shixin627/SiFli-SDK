...
void lvsf_gesture_bars_realign(void)
{
    if (gesture_detect_objs[0])
    {
        ...
        // lv_obj_align(gesture_detect_objs[0], LV_ALIGN_OUT_LEFT_MID,
        // LVSF_GESTURE_DETECT_WIDTH, 0);
...
static bool back_hint_vibrated =
    false; // Flag to ensure vibration happens only once
...
static void back_hint_anim_cb(void *obj, uint16_t x)
{
    if (lv_obj_is_valid(obj))
    {
        ...
        {
            uint16_t obj_width =
                LVSF_GESTURE_BACK_HINT_SIZE * x / LVSF_GESTURE_LIMIT;
            lv_obj_set_width(obj, obj_width);
            lv_obj_set_style_radius(obj, 25, 0);
            lv_obj_align(obj, LV_ALIGN_LEFT_MID, 0, 0);
            if (obj_width > 30)
            {
                lv_obj_set_style_img_opa(gesture_detect_hint_icon, LV_OPA_COVER,
                                         0);
            }
            else
            {
                lv_obj_set_style_img_opa(gesture_detect_hint_icon,
                                         LV_OPA_TRANSP, 0);
            }

            // Trigger vibration when going back below threshold (from above
            // threshold)
        ...
        else
        {
            ...
            // Trigger vibration when circle is fully formed (crossing threshold
            // upward)
            if (!back_hint_vibrated)
            {
...
static void left_bar_event_handler(lv_event_t *e)
{
    ...
    else if (event == LV_EVENT_RELEASED)
    {
        ...
        if (LV_ABS(start_point.x - end_point.x) > LVSF_GESTURE_LIMIT)
        {
            ...
            lv_anim_set_exec_cb(
                &back_hint_release_anim,
                (lv_anim_exec_xcb_t)set_gesture_detect_hint_obj_opa);
...
void lvsf_gesture_init(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_bg_opa(bar, LV_OPA_TRANSP,
