...
static void lv_baseanim_init_snapshot_dsc(lv_img_dsc_t *img_dsc, uint8_t index, uint16_t align)
{
    ...
    img_dsc->header.stride = w * lv_color_format_get_size(LV_IMG_CF_TRUE_COLOR);
    img_dsc->header.flags = 0;
    img_dsc->header.cf = LV_IMG_CF_TRUE_COLOR;
    img_dsc->header.w = w;
    img_dsc->header.h = h;
    img_dsc->data = trans_anim_buf;
    img_dsc->data_size = lv_color_format_get_size(LV_IMG_CF_TRUE_COLOR) * w * h;
}
...
static void lv_baseanim_init_fb_dsc(lv_img_dsc_t *img_scr, lv_disp_t *disp)
{
    ...
    img_scr->header.magic = LV_IMAGE_HEADER_MAGIC;
    img_scr->header.flags = 0;
    img_scr->header.stride = hor_res * lv_color_format_get_size(LV_COLOR_FORMAT_NATIVE);
    img_scr->header.w = hor_res;
    img_scr->header.h = ver_res;
    img_scr->data_size = (LV_COLOR_DEPTH * hor_res * ver_res) / 8;
    img_scr->header.cf = LV_COLOR_FORMAT_NATIVE;
    int32_t draw_size = hor_res * ver_res * lv_color_format_get_size(img_scr->header.cf);
