...
#endif
    {
        extern int32_t keypad_do_event(lv_key_t key, lv_indev_state_t event);
...
void lv_hal_init(const char *name)
{
    if (!lv_win32_init(
                ...
                LV_HOR_RES_MAX,
                LV_VER_RES_MAX,
                LoadIconW(GetModuleHandleW(NULL), (LPCWSTR)MAKEINTRESOURCE(IDI_APPLICATION))))
    {
        return;
    }
    ...
    {
        extern void lv_win32_imgdec_init(void);
        lv_win32_imgdec_init();
    }
#endif /* RT_USING_DFS */
}
