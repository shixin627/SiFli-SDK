...
typedef struct bt_voice_tag
{
    ...
    sco_ipc_t  *sco_ipc;
    uint8_t sco_num;
    uint8_t air_mod[BT_SCO_MAX_NUM];
    uint8_t state;
    uint8_t sco_state[BT_SCO_MAX_NUM];
    uint8_t send_enable;
    uint8_t is_ready;
    uint8_t sn_cnt[BT_SCO_MAX_NUM];
} bt_voice_t;
...
static uint8_t bt_voice_relay_chk_datahdr(uint32_t hdr)
{
    ...
    // LOG_I("bt_voice_chk_datahdr 0x%2x", sco_hdr->scohdl);
    //only support 60byte data length
    if ((sco_hdr->length == 0x3c) && (scohdl_l <= 4) && (scohdl_m == 8) && (scohdl_h >= 1) && (scohdl_h <= BT_SCO_MAX_NUM))
    {
        hdr_ok = 1;
    }
...
static uint16_t bt_voice_relay_sco_get_peer_hdl(uint8_t idx, uint16_t cur_hdl)
{
    ...
    {
        return pt_bt_voice->dn_data[peer_idx].handle;
    }

    if ((peer_idx < BT_SCO_MAX_NUM) && pt_bt_voice->header[peer_idx].scohdl)
    {
        return pt_bt_voice->header[peer_idx].scohdl;
    }
...
static uint8_t bt_voice_relay_msbc_reencode_to_payload(uint8_t sco_idx, uint8_t *pcm, uint8_t *payload)
{
    ...
    payload[0] = 0x01;
    payload[1] = msbc_sn[pt_bt_voice->sn_cnt[sco_idx]++];
    if (pt_bt_voice->sn_cnt[sco_idx] == 4)
    {
        pt_bt_voice->sn_cnt[sco_idx] = 0;
    }

    pbss_t.psrc = pcm;
    pbss_t.src_len = AUDIO_MSBC_BUFFER_LEN;
    pbss_t.dst_len = 57;
...
static uint8_t bt_voice_relay_cvsd_reencode_to_payload(uint8_t sco_idx, uint8_t *pcm, uint8_t *payload)
{
    ...
    memcpy(pt_cvsd->inp_buf_shift, pcm, AUDIO_BT_CVSD_PCM_LEN);

    interpolation_x8(pt_cvsd->inp_buf, pt_cvsd->buf_size_FIR_assumpt, pt_cvsd->interpolate_buf, pt_cvsd->out_len_interpolate);
    cvsdEncode(&(pt_cvsd->cvsd_e), (const int16_t *)pt_cvsd->interpolate_buf, pt_cvsd->out_len_interpolate, (uint32_t *)payload);
...
#else
    (void)sco_idx;
    memcpy(payload, pcm, BT_CVSD_FRAME_LEN);
    return BT_CVSD_FRAME_LEN;
#endif
}
...
static uint8_t bt_voice_relay_reencode_and_send(uint8_t sco_idx, uint32_t local_datahdr, uint32_t peer_datahdr)
{
    ...
    {
        LOG_I("pt_mic2bt_rbf full\n");
        return 0;
    }

    *(uint32_t *)(&packet[0]) = peer_datahdr;

    if (!peer_state)
    {
        *(uint32_t *)(&packet[0]) = local_datahdr;
        peer_idx = sco_idx;
    }
    ...
    {
        LOG_W("3a_w relay reencode err sco=%d, payload_len=%d\n", sco_idx, payload_len);
        return 0;
    }

    putnum = rt_ringbuffer_put(g_sco_ipc.pt_mic2bt_rbf, packet, sizeof(packet));
    if (putnum != sizeof(packet))
    {
        LOG_I("pt_mic2bt_rbf put err %d\n", putnum);
        return 0;
    }
...
void bt_voice_relay_downlink_process(uint8_t is_ready)
{
    ...
    if ((pt_bt_voice == NULL) || (pt_bt_voice->is_ready == 0) || (is_ready == 0))
    {
        return;
    }
    ...
    while (rt_ringbuffer_data_len(g_sco_ipc.pt_bt2speaker_rbf) > size)
    {
        ...
        if (find_hdr == 1)
        {
            ...
            pt_dn_data = &pt_bt_voice->dn_data[sco_path];
...
void bt_voice_relay_open_sco(uint8_t sco_idx, uint32_t samplerate)
{
    ...
    {
        cvsd_g711plc_construct(&(pt_bt_voice->pcm_plc[sco_idx]));
    }
    else
    {
        // RT_ASSERT(bt_voice_get_open_sco_count() == 0);
    ...
    if (0 == pt_bt_voice->is_ready)
    {
        pt_bt_voice->is_ready = 1;
    }
}
...
void bt_voice_relay_close_sco(uint8_t sco_idx)
{
    ...
    if ((pt_bt_voice == NULL) || (pt_bt_voice->sco_state[sco_idx] == 0))
    {
        return;
    }
    ...
    bts2_msbc_encode_completed_ext(sco_idx);
    bts2_msbc_decode_completed_ext(sco_idx);
    memset(&pt_bt_voice->dn_data[sco_idx], 0, sizeof(sco_data_t));
    memset(&pt_bt_voice->header[sco_idx], 0, sizeof(bt_sco_data_hdr));
