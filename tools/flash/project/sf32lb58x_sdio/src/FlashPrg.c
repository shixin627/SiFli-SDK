...
{
    while ((hal_sdhci_get_int_value(&sd1_handle) & SDHCI_INT_CMD_MASK) == 0);
    return (hal_sdhci_get_int_value(&sd1_handle) & SDHCI_INT_CMD_MASK);
}

void sd_cfg_write(uint8_t wire_mode, uint8_t block_num)
{
    hal_sdhci_set_bus_width(&sd1_handle, wire_mode);
    hal_sdhci_set_blk(&sd1_handle, 512, 1);
...
{
    while ((hal_sdhci_get_int_value(&sd1_handle) & SDHCI_INT_DATA_MASK) == 0);
    return (hal_sdhci_get_int_value(&sd1_handle) & SDHCI_INT_DATA_MASK);
}

void sd_cfg_read(uint8_t wire_mode, uint8_t block_num, uint16_t block_size)
{
    hal_sdhci_set_bus_width(&sd1_handle, wire_mode);
    hal_sdhci_set_blk(&sd1_handle, block_size, 1);
    //sd_wr_byte(SDHCI_HOST_CONTROL,((wire_mode==2)?SDHCI_CTRL_8BITBUS:(wire_mode==1)?SDHCI_CTRL_4BITBUS:0));
    //sd_wr_hword(SDHCI_BLOCK_SIZE,SDHCI_MAKE_BLKSZ(0,SD_BLOCK_SIZE));
    //sd_wr_hword(SDHCI_BLOCK_COUNT,block_num);
}
...
void sd_clr_status()
{
    hal_sdhci_clear_int(&sd1_handle, SDHCI_INT_ALL_MASK);
}

void sd_get_rsp(uint32_t *rsp_arg1, uint32_t *rsp_arg2, uint32_t *rsp_arg3, uint32_t *rsp_arg4)
{
    *rsp_arg1 = hal_sdhci_get_response(&sd1_handle, 0);
    *rsp_arg2 = hal_sdhci_get_response(&sd1_handle, 1);
    *rsp_arg3 = hal_sdhci_get_response(&sd1_handle, 2);
    *rsp_arg4 = hal_sdhci_get_response(&sd1_handle, 3);
}
...
uint32_t sd_send_cmd(uint8_t cmd_idx, uint32_t cmd_arg)
{
    ...
    if (with_data)
        cmd_para |= SDHCI_CMD_DATA;

    //write command register
    cmd.Arg = cmd_arg;
    cmd.CmdIndex = cmd_idx;
    cmd.RespType = cmd_para;
    hal_sdhci_send_command(&sd1_handle, &cmd);
...
uint8_t sdmmc_emmc()
{
    ...
    debug_print("Init sdmmc\r\n");
#endif

    //initialize sdmmc host
    hal_sdhci_set_power(&sd1_handle, 0x107);

    //hal_sdhci_set_clk(&sd1_handle,400000, 24000000);
    extern void hal_sdhci_clk_div(SDHCI_HandleTypeDef * handle, uint32_t div);
    hal_sdhci_clk_div(&sd1_handle, 64);
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
    debug_print("before set power\r\n");
#endif

    hal_sdhci_set_power(&sd1_handle, 0x10);; //push-pull mode for cmd line

    //start card transfer
    //CMD9
#if (DEBUG_JLINK+DEFAULT_TRACE)
    debug_print("before cmd 9\r\n");
#endif
    ...
    sd_clr_status();
    cmd6_index = 183;
    cmd6_value = 1; //0 for sdr 1-wire, 1 for sdr 4-wire, 2 for sdr 8-wire
    cmd_arg = 0x03000000 | (cmd6_index << 16) | (cmd6_value << 8);
    cmd_result = sd_send_cmd(6, cmd_arg);
    sd_clr_status();

#if (DEBUG_JLINK+DEFAULT_TRACE)
    debug_print("before set clock\r\n");
#endif

    //switch to fast clock
    hal_sdhci_set_clk(&sd1_handle, 48000000, 48000000);
    hal_sdhci_set_ddr(&sd1_handle, 0);
...
int sd_read_data(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    hal_sdhci_set_transfer_mode(&sd1_handle, SDHCI_TRNS_BLK_CNT_EN | SDHCI_TRNS_READ);
    //sd_wr_hword(SDHCI_TRANSFER_MODE,SDHCI_TRNS_BLK_CNT_EN|SDHCI_TRNS_READ);
    if (len >= 512)
        blk_size = 512;
    else
        blk_size = len;
    sd_cfg_read(4, 1, blk_size); //4 wire mode,single block
    cmd_arg = addr; //0x800; //start data address
    sd_clr_status();
    cmd_result = sd_send_cmd(17, cmd_arg);
    if (cmd_result != SDHCI_INT_RESPONSE)
    {
        debug_print("wait cmd 17 respons fail\r\n");
        return 0;
    }
    ...
    {
        debug_print("Wait read data valid fail\r\n");
        return 0;
    }
    sd_clr_status();

    hal_sdhci_transfer_pio(&sd1_handle, 1, data, blk_size);

    //check transfer complete flag
    cmd_result = sd_wait_read();
    if (cmd_result != SDHCI_INT_DATA_END)
    {
        debug_print("Wait read done fail\r\n");
        return 0;
    }
    sd_clr_status();
...
int sd_write_data(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    //write data
    hal_sdhci_transfer_pio(&sd1_handle, 0, data, blk_size);

    cmd_result = sd_wait_write();
    if (cmd_result != SDHCI_INT_DATA_END)
    {
        debug_print("wait write done fail\r\n");
        return 0;
    }
    sd_clr_status();
