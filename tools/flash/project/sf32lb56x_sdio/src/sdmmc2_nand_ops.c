...
uint8_t sdmmc2_memory_init()
{
  ...
  cmd_result = sd2_send_cmd(8,cmd_arg); //CMD8
  ...
  while(1) { //wait for card busy status
    cmd_result = sd2_send_acmd(41,cmd_arg,rca); //CMD55+ACMD41
    if (cmd_result == SD_TIMEOUT) {
  ...
  hwp_sdmmc2->CLKCR = 1 << SD_CLKCR_DIV_Pos; //48M/2=24M
  hwp_sdmmc2->CLKCR |= SD_CLKCR_VOID_FIFO_ERROR;
  ...
  cmd_result = sd2_send_acmd(6,cmd_arg,rca); //CMD55+ACMD6
  if (cmd_result == SD_TIMEOUT) {
  ...
  {
    debug_print("SD2 init pass\n");
  }
  else 
  {
    debug_print("SD2 init fail\n");
  }
...
int sd_read_data(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    //debug_print("\r\n"); 
      ...
      hwp_sdmmc2->CASR = SD_CASR_SD_BUSY;
    
      //select card type
      if (sdsc == 0) {
        hwp_sdmmc2->CACR &= ~SD_CACR_CACHE_SDSC; //for SDHC/SDXC
      }
