...
int sdmmc2_memory_init(void)
{
    ...
    hwp_sdmmc2->CDR = (0<<SD_CDR_ITIMING_Pos);
    ...
    for (i = 0; i < 512 / 4; i++)
    {
        sdemmc_cache[i] = hwp_sdmmc2->FIFO;
    }
    // parse CSD
    ...
    sd2_read(wire_mode, 1); //4 wire mode,1 blocks
#else 
    sd2_read(0, 1); //1 wire mode,1 blocks
#endif
...
int sd_read_data(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    //debug_print("\r\n"); 
      //dma config for read
      //hwp_dmac1->CPAR5  = &hwp_sdmmc2->FIFO;
      ...
      //hwp_sdmmc2->FDLR = 64 | SD_FDLR_RD_VALID_EN; //dma request threshold is 128 word (one block)
    
      //start data read before read command
      sd2_read(wire_mode,1); //4 wire mode,8 blocks
...
int sd_write_data(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    //start dma for write
    sd2_write(wire_mode,1); //4 wire mode,1 block
    hwp_sdmmc2->SR = 0xffffffff; //clear sdmmc interrupts
    hwp_sdmmc2->IER = SD_IER_DATA_DONE_MASK;
