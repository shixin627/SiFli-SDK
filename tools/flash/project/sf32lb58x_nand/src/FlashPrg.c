...
int rt_hw_flash4_init()
{
    ...
    spi_flash_handle[3].handle.data_buf_w = (uint8_t *)&nand_buf[0];
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
    uint8_t hex[16];
    debug_print("\r\nNAND flash controller id: ");
    debug_print((char *)htoa(hex, nand_index));
    debug_print("\r\n");
#endif
    if (nand_index > 0)
    {
...
#if 0
#if (DEBUG_JLINK+DEFAULT_TRACE)
        uint32_t total, bbmctx;
        total = bbm_get_total();
        bbmctx = (uint32_t)bbm_get_context();
        debug_print("NAND total 0x");
        debug_print((char *)htoa(hex, total));
        debug_print(" ,bbm ctx buffer pointer 0x");
        debug_print((char *)htoa(hex, bbmctx));
        debug_print("\r\n");
#endif
#endif
    }
...
void init_clock()
{
    ...
    BSP_SetFlash4DIV(3);    // src 216, div 3, output 72m
}
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
    if(res != (SPI_NAND_PAGE_SIZE+spare_len))
    {
        uint8_t hex[16];
...
int bbm_get_bb(int blk)
{
    ...
    {
        uint8_t hex[16];
        debug_print("Check block ");
        debug_print((char *)htoa(hex, blk));
        debug_print(" is BAD!!! Mark value 0x");
