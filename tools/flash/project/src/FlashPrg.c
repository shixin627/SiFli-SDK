...
static FLASH_HandleTypeDef *Addr2Handle(uint32_t addr)
{
    ...
    if ((addr >= spi_flash_handle[0].base_addr) && (addr < spi_flash_handle[0].base_addr + spi_flash_handle[0].total_size))
        id = 0;
    else if ((addr >= spi_flash_handle[1].base_addr) && (addr < spi_flash_handle[1].base_addr + spi_flash_handle[1].total_size))
        id = 1;
    else if ((addr >= spi_flash_handle[2].base_addr) && (addr < spi_flash_handle[2].base_addr + spi_flash_handle[2].total_size))
        id = 2;
    else if ((addr >= spi_flash_handle[3].base_addr) && (addr < spi_flash_handle[3].base_addr + spi_flash_handle[3].total_size))
        id = 3;
    else // not defined flash3 base address, set default to flash2
        id = -1;
...
int rt_hw_flash4_init(uint8_t auto_detect)
{
    ...
    uint8_t hex[16];
    debug_print("FLASH4 Bonding ID: ");
    debug_print((char *)htoa(hex, pid));
    debug_print("\r\n");
#endif
    hwp_lpsys_rcc->ENR2 |= LPSYS_RCC_ENR2_QSPI4_Msk;
    
    if (pid != LB557_BID)
    {
        // defalt function setting in pinmux is based on 555 evb
        HAL_PIN_SetFlash4();
    }
    else
    {
        // 557 pinmux setting for evb
        HAL_PIN_Set(PAD_PB00, QSPI4_CLK, PIN_NOPULL, 0);        // QSPI4
        HAL_PIN_Set(PAD_PB01, QSPI4_CS, PIN_NOPULL, 0);     // it conflict with 555 qspi4 en
        HAL_PIN_Set(PAD_PB02, QSPI4_DIO0, PIN_NOPULL, 0);
    ...
    res = HAL_FLASH_Init(&(spi_flash_handle[3]), &flash_cfg4, &spi_flash_dma_handle[3], &flash_dma4, 6);
#if (DEBUG_JLINK+DEFAULT_TRACE)
    if (res == HAL_OK)
        debug_print("\rFLASH4 Init success\r\n");
    else
        debug_print("\rFLASH4 Init fail\r\n");
#endif
...
static int rt_hw_flash_init(void)
{
...
#if !defined(JLINK) || defined(JLINK_FLASH_1)
#ifdef BSP_ENABLE_QSPI1 // add qspi check to make sure config table exist
...
#ifdef DUAL_FLASH_AUTO_DETECT
    extern __HAL_ROM_USED int HAL_FLASH_DETECT_DUAL(QSPI_FLASH_CTX_T * ctx, qspi_configure_t *cfg);
    HAL_PIN_Set_Dual_flash1();
    dual = HAL_FLASH_DETECT_DUAL(&(spi_flash_handle[0]), &flash_cfg);
    if (dual)
        spi_flash_handle[0].dual_mode = 2;  // dual mode
    else
    {
        spi_flash_handle[0].dual_mode = 1;  // single mode
        HAL_PIN_Set_Dual_flash1_default();
    }
#if (DEBUG_JLINK+DEFAULT_TRACE)
    if (dual)
        debug_print("\rFLASH1 DUAL MODE\r\n");
    else
        debug_print("\rFLASH1 SINGLE MODE\r\n");
#endif
#endif  //DUAL_FLASH_AUTO_DETECT
...
#endif //!defined(JLINK) || defined(JLINK_FLASH_1)

#if !defined(JLINK) || defined(JLINK_FLASH_2)
#ifdef BSP_ENABLE_QSPI2 // add qspi check to make sure config table exist
    ...
    HAL_PIN_Set_Single_flash2();

#ifdef DUAL_FLASH_AUTO_DETECT
    extern __HAL_ROM_USED int HAL_FLASH_DETECT_DUAL(QSPI_FLASH_CTX_T * ctx, qspi_configure_t *cfg);

    HAL_PIN_Set_Dual_flash2();
    dual = HAL_FLASH_DETECT_DUAL(&(spi_flash_handle[1]), &flash_cfg2);
    if (dual)
        spi_flash_handle[1].dual_mode = 2;  // dual mode
    else
    {
        spi_flash_handle[1].dual_mode = 1;  // single mode
        HAL_PIN_Set_Dual_flash2_default();
    }
#if (DEBUG_JLINK+DEFAULT_TRACE)
    if (dual)
        debug_print("\rFLASH2 DUAL MODE\r\n");
    else
        debug_print("\rFLASH2 SINGLE MODE\r\n");
#endif

#endif  //DUAL_FLASH_AUTO_DETECT

    // init hardware, set dma, clock
    res = HAL_FLASH_Init(&(spi_flash_handle[1]), &flash_cfg2, &spi_flash_dma_handle[1], &flash_dma2,   BSP_GetFlash2DIV());
    if (res == HAL_OK)
        fen |= FLASH2_ENABLED;
#endif  // BSP_ENABLE_QSPI2
#endif //!defined(JLINK) || defined(JLINK_FLASH_2)
    ...
    //spi_flash_handle[2].dual_mode = 0;  // single mode
#if (DEBUG_JLINK+DEFAULT_TRACE)
    debug_print("\rFLASH3 SINGLE MODE\r\n");
#endif
    ...
    if (res == HAL_OK)
    {
        if (spi_flash_handle[2].handle.isNand == 1)
        {
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
        if (spi_flash_handle[2].flash_mode == 0)
            debug_print("FLASH3 NOR MODE\r\n");
        else
            debug_print("FLASH3 NAND MODE\r\n");
#endif
        fen |= FLASH3_ENABLED;
    }
...
void init_clock()
{
    ...
    //HAL_RCC_HCPU_ClockSelect(RCC_CLK_MOD_FLASH2, RCC_CLK_SRC_DLL2);
    BSP_SetFlash1DIV(1);
...
__attribute__((used)) int SEGGER_OPEN_Program(uint32_t DestAddr, uint32_t NumBytes, uint8_t *pSrcBuff)
{
    ...
    debug_print("\r\n");
#endif
    //SCB_InvalidateDCache_by_Addr((void *)pSrcBuff, NumBytes);
    ...
    if (fhandle->isNand)
    {
        ...
        if (len > 0)
        {
            ret += HAL_QSPIEX_FLASH_WRITE(Addr2Handle(dst), dst, src, len);
            //len -= len;
            dst += len;
            src += len;
        }
        ...
        ret = HAL_QSPIEX_FLASH_WRITE(Addr2Handle(DestAddr), DestAddr, pSrcBuff, NumBytes);
    //SCB_InvalidateDCache_by_Addr((void *)DestAddr, NumBytes);
...
__attribute__((used)) int SEGGER_OPEN_Erase(uint32_t SectorAddr, uint32_t SectorIndex, uint32_t NumSectors)
{
    ...
    int res = HAL_QSPIEX_FLASH_ERASE(Addr2Handle(SectorAddr), SectorAddr, NumSectors * PAGE_SIZE);
...
#ifdef JLINK_FLASH_3
    //uint8_t nand_cache[131072];
    uint8_t nand_cache[2048];
#endif
uint32_t SEGGER_OPEN_CalcCRC(uint32_t crcs, uint32_t Addr, uint32_t NumBytes, uint32_t Polynom)
{
    FLASH_HandleTypeDef *fhandle = Addr2Handle(Addr);
#ifdef JLINK_FLASH_3
    uint8_t   *pCrcData = (uint8_t *)(&nand_cache[0]);
#else
    uint8_t   *pCrcData = (uint8_t *)Addr; //NULL; //(uint8_t*)Addr;
#endif
    ...
    if (fhandle)
    {
        ...
        {
#ifdef JLINK_FLASH_3    // only for jlink flash3, keil not support nand, other jlink should modify source code
            uint32_t len = NumBytes;
            uint8_t *dst = pCrcData;
            uint32_t src = Addr;
            int ret = 0;
            ...
            while (len >= 2048)
            {
                ...
                {
                    crcs = (crcs >> 8) ^ pCrcTable[(crcs & 0xFF) ^ pCrcData[k]];
                }
            }
            if (len > 0)
            {
