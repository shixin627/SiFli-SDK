...
int rt_hw_flash3_init()
{
    ...
    spi_flash_handle[2].handle.freq = 120000000 / div;
    ...
    { 
        ...
        debug_print((char *)htoa(hex, spi_flash_handle[2].total_size));
        debug_print("\r\n");
#endif
...
__attribute__((used)) int SEGGER_OPEN_Erase(uint32_t SectorAddr, uint32_t SectorIndex, uint32_t NumSectors)
{
    ...
    debug_print((char *)htoa(hex, PAGE_SIZE * NumSectors));
	  ...
	  if (0 != (SectorAddr % nand_blksize))
		{
			  /* align to block boundary */
        return 0;
		}

		/* erase at least one block if at block boundary */
    bcnt = (PAGE_SIZE * NumSectors + nand_blksize - 1) / nand_blksize; // to real block counter
	
#if DEBUG_JLINK
	  debug_print("\n");
	debug_print("erase blk cnt: ");
    debug_print((char *)htoa(hex, bcnt));
#endif
...
int port_read_page(int blk, int page, int offset, uint8_t *buff, uint32_t size, uint8_t *spare, uint32_t spare_len)
{
    ...
    if(res != (size+spare_len))
    {
        ...
        debug_print((char *)htoa(hex, hflash->ErrorCode));
			  debug_print(", ");
			  debug_print((char *)htoa(hex, hflash->dualFlash));
			  debug_print(", ");
			  debug_print((char *)htoa(hex, size));
  			debug_print("\r\n"); 
    }
#endif
...
#if (DEBUG_JLINK)
    uint8_t hex[16];
		debug_print("get bad blk: 0x");
    debug_print((char *)htoa(hex, blk));
		debug_print(",0x");
		debug_print((char *)htoa(hex, bad));
    debug_print("\r\n"); 
#endif		
