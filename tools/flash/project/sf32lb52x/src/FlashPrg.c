...
int rt_hw_flash1_init()
{
    ...
    qspi_configure_t flash_cfg1 = FLASH1_CONFIG;
    struct dma_config flash_dma1 = FLASH1_DMA_CONFIG;
    uint16_t div = BSP_GetFlash1DIV();

    uint32_t pid = (hwp_hpsys_cfg->IDR & HPSYS_CFG_IDR_PID_Msk) >> HPSYS_CFG_IDR_PID_Pos;

    flash_cfg1.line = HAL_FLASH_NOR_MODE;
    if (pid == 0)
    {
    ...
    else if (pid == 1)
    {
        ...
        flash_cfg1.line = HAL_FLASH_QMODE;
    }
    else 
    {
#if (DEBUG_JLINK+DEFAULT_TRACE)
        uint8_t hex[16];
        debug_print("\r\nInvliad pid for SIP 1 flash: ");
    ...
    //flash_cfg1.base = HCPU_MPI_SBUS_ADDR(flash_cfg3.base);
    // init hardware, set dma, clock
    res = HAL_FLASH_Init(&(spi_flash_handle[0]), &flash_cfg1, &spi_flash_dma_handle[0], &flash_dma1, div);
#if (DEBUG_JLINK+DEFAULT_TRACE)
    uint8_t hex[16];
    debug_print("\r\nFLASH1 INIT : ID 0x");
    debug_print((char *)htoa(hex, spi_flash_handle[0].dev_id));
    debug_print(" ,res ");
    ...
    if (res == HAL_OK)
    {
        if(pid == 0)    // for puya, need configure dio2 for diff memory
        {
...
#endif //#if !defined(JLINK) || defined(JLINK_FLASH_1)
...
{
    uint8_t  *pCrcData = (uint8_t *)Addr;
    uint32_t  *pCrcTable = NULL;

#if (DEBUG_JLINK)
    uint8_t hex[16];
    debug_print("CalcCRC : crcs-");
