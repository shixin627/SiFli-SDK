...
static char nand_buf[4096 + SPI_NAND_MAXOOB_SIZE];
...
static uint32_t nand_blksize = 0x20000;
    ...
    static uint8_t bbm_cache_buf[4096 + SPI_NAND_MAXOOB_SIZE];
#endif
...
int rt_hw_flash2_init()
{
    ...
    res = HAL_FLASH_Init(&(spi_flash_handle[1]), &flash_cfg2, &spi_flash_dma_handle[1], &flash_dma2, div);
#if (DEBUG_JLINK+DEFAULT_TRACE)
    uint8_t hex[16];
    debug_print("\r\nFLASH2 INIT : ID 0x");
    debug_print((char *)htoa(hex, spi_flash_handle[1].dev_id));
    debug_print(" ,res ");
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
        uint8_t hex[16];
        debug_print("NAND PAGE SIZE : 0x");
        debug_print((char *)htoa(hex, nand_pagesize));
        debug_print(", BLK SIZE 0x");
        debug_print((char *)htoa(hex, nand_blksize));
        debug_print(", TOTAL SIZE 0x");
        debug_print((char *)htoa(hex, spi_flash_handle[1].total_size));
        debug_print("\r\n");
#endif
...
#endif //#if !defined(JLINK) || defined(JLINK_FLASH_2)
...
void init_clock()
{
    ...
    BSP_SetFlash1DIV(2);    // src 120, div 2, output 60m
    BSP_SetFlash2DIV(2);
}
...
__attribute__((used)) int EraseSector(uint32_t SectorAddr)
{
    ...
    int res = HAL_NAND_GET_BADBLK(Addr2Handle(SectorAddr), (addr / nand_blksize));
...
__attribute__((used)) int SEGGER_OPEN_Read(uint32_t Addr, uint32_t NumBytes, uint8_t *pDestBuff)
{
    ...
    if (fhandle->isNand == 0)
    {
        //return 0; // this is nand driver code
        memcpy(pDestBuff, (uint8_t *)Addr, NumBytes);
    }
    else
    {
        if ((Addr & (nand_pagesize-1)) != 0)
            return 0;
        uint32_t len = NumBytes;
        uint8_t *dst = pDestBuff;
        uint32_t src = Addr;

        while (len >= nand_pagesize)
        {
#ifdef BSP_USING_BBM
            int blk, page, offset;
            uint32_t addr;
            ...
            page = (addr % nand_blksize ) / nand_pagesize;
            offset = addr & (nand_pagesize -1);
            ret = bbm_read_page(blk, page, offset, dst, nand_pagesize, NULL, 0);
#else
            ret = HAL_NAND_READ_WITHOOB(fhandle, src, dst, nand_pagesize, NULL, 0);
#endif
...
__attribute__((used)) int SEGGER_OPEN_Program(uint32_t DestAddr, uint32_t NumBytes, uint8_t *pSrcBuff)
{
    ...
    if (fhandle->isNand)
    {
        ...
        while (len >= nand_pagesize)
        {
            ...
            if(i == nand_pagesize/4) // full page data is 0xffffffff, do not need write to flash
            {
#if (DEBUG_JLINK)
            ...
            ret += HAL_NAND_WRITE_WITHOOB(Addr2Handle(dst), dst, src, nand_pagesize, NULL, 0);
#endif
            len -= nand_pagesize;
            dst += nand_pagesize;
            src += nand_pagesize;
...
__attribute__((used)) int SEGGER_OPEN_Erase(uint32_t SectorAddr, uint32_t SectorIndex, uint32_t NumSectors)
{
    ...
    debug_print(" Size-");
    debug_print((char *)htoa(hex, PAGE_SIZE * NumSectors));
#endif

    bcnt = PAGE_SIZE * NumSectors / nand_blksize; // to real block counter
#ifdef BSP_USING_BBM
    int blk;
    uint32_t addr;
    ...
    for (i = 0; i < bcnt; i++)
    {
        res = HAL_NAND_ERASE_BLK(Addr2Handle(SectorAddr), SectorAddr + i * nand_blksize);
        if (res) // erase fail
            break;
    }
#endif
...
#define REPLACE_SMALL_BUF
#ifdef REPLACE_SMALL_BUF
static uint8_t nand_cache[4096];
#else
...
uint32_t SEGGER_OPEN_CalcCRC(uint32_t crcs, uint32_t Addr, uint32_t NumBytes, uint32_t Polynom)
{
...
#ifdef REPLACE_SMALL_BUF
    uint32_t remain, taddr, fill, tcnt;
#endif
#if (DEBUG_JLINK)
    uint8_t hex[16];
    ...
    if (fhandle)
    {
        if (fhandle->isNand) // nand
        {
            ...
            taddr = Addr;
#else 
            SEGGER_OPEN_Read(Addr, NumBytes, pCrcData);
#endif
        }
        else // nor
    ...
    if (fhandle->isNand)
    {
        while(remain > 0)
        {
            fill = remain > nand_pagesize ? nand_pagesize : remain;
            SEGGER_OPEN_Read(taddr, fill, pCrcData);
            tcnt = fill;
            while (tcnt--)
            {
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
    if(res != (size+spare_len))
    {
        uint8_t hex[16];
        debug_print("page read error : addr 0x");
        debug_print((char *)htoa(hex, addr));
        debug_print(", err code 0x");
        debug_print((char *)htoa(hex, hflash->ErrorCode));
        debug_print("\r\n"); 
    }
#endif
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
    if(res != (nand_pagesize+spare_len))
    {
        uint8_t hex[16];
        debug_print("page write error : addr 0x");
        debug_print((char *)htoa(hex, addr));
        debug_print(", err code 0x");
        debug_print((char *)htoa(hex, hflash->ErrorCode));
        debug_print("\r\n"); 
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
        uint8_t hex[16];
        debug_print("Erase block, bad block at blk 0x");
        debug_print((char *)htoa(hex, blk));
        debug_print("\n");
#endif
        return -1; //-RET_ERROR;
...
#if (DEBUG_JLINK+DEFAULT_TRACE)
        uint8_t hex[16];
        debug_print("blk erase error : blk 0x");
        debug_print((char *)htoa(hex, blk));
        debug_print(", err code 0x");
        debug_print((char *)htoa(hex, hflash->ErrorCode));
        debug_print("\r\n"); 
