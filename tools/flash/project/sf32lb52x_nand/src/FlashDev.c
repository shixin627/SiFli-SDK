...
#ifdef JLINK_FLASH_2
    #define FLASH_NAME  "SF32LB52X External Nand2"
    #define FLASH_BASE  0x62000000
    #define FLASH_SIZE  0x3e000000
#endif
...
struct FlashDevice const FlashDevice __attribute__((section("DevDscr"), used)) =
{
    ...
    FLASH_WRITE_BASE_SIZE,     // Affect the FLASH write action. Page Size (number of bytes that will be passed to ProgramPage(). May be multiple of min alignment in order to reduce overhead for calling ProgramPage multiple times
...
{
    ALGO_VERSION,              // Algo version
    "SF32LB54X UNI_FLASH",    // Flash device name
    ONCHIP,                    // Flash device type
