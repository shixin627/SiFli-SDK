...
def VS2017_CreateFilter(script, project_path):
    ...
    for group in script:
        VS2017_AddGroup(c_ItemGroup, group['name'], group['src'], project_path)
        Filter = SubElement(filter_ItemGroup, 'Filter')
...
def VS2017Project(target, script, program):
    project_path = os.path.dirname(os.path.abspath(target))
    
    tree = etree.parse('template_vs2017.vcxproj')
    root = tree.getroot()
    elem = root
    ...
    VS2017_CreateFilter(script, project_path)
    # add "*.c" files
    for group in script:
    ...
    if 'CPPDEFINES' in building.Env:
        for elem in tree.iter(tag='PreprocessorDefinitions'):
            CPPDEFINES = []
            for i in building.Env['CPPDEFINES']:
                if type(i) is str:
                    CPPDEFINES += [i]
                else:
                    CPPDEFINES += [i[0]]
            definitions = ';'.join(CPPDEFINES) + ';%(PreprocessorDefinitions)'
