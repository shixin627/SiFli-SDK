...
def _main():    
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Do not show dialog, update only")
    parser.add_argument(
        "--conf", 
        type=str,
        default='.config',
    ...
    kconfig_file=args.conf
    if (kconfig_file=='.config'):
        if (os.path.isfile('proj.conf')):
            kconfig_file='proj.conf'
        elif (os.path.isfile('board.conf')):
            kconfig_file='board.conf'

    board_conf_path = None
    board_path = None
    if kconfig_file == "proj.conf" and args.board:
        ...
        args.kconfig = "Kconfig.tmp"
        with open(args.kconfig, "w") as f:
            # f.write('choice\n')
            # f.write('    prompt "Board Selection"\n')
            f.write('source "{}/Kconfig.board"\n'.format(board_path.replace("\\", "/")))
            # f.write("endchoice\n")
            # f.write("endmenu\n")
            f.write('source "$SIFLI_SDK/customer/boards/Kconfig.v2"\n')
            f.write('source "$SIFLI_SDK/Kconfig.v2"\n')
            f.write('orsource "sf-pkgs/Kconfig.conandeps"\n')
    ...
    menuconfig(standard_kconfig(args.kconfig), kconfig_file)


def menuconfig(kconf,conf='.config'):
    """
    ...
    if (args.silent==True):
        _kconf.write_config(_conf_filename)
        # save header file
        _kconf.write_autoconf()        
    else:
...
def _warning_board():
    while True:
        c = _key_dialog(
            "Warning",
            " Current config file is board.conf or proj.conf, \n"
            " use save minimal config instead\n"
            "(C)ancel",
            "c")
        if c is None or c == "c":
            return None
