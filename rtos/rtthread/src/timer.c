...
/* hard timer list */
__ROM_USED rt_list_t rt_timer_list[RT_TIMER_SKIP_LIST_LEVEL];
    ...
    L1_NON_RET_BSS_SECT_BEGIN(timer_thread_stack)
    ALIGN(RT_ALIGN_SIZE)
    L1_NON_RET_BSS_SECT(timer_thread_stack, static rt_uint8_t timer_thread_stack[RT_TIMER_THREAD_STACK_SIZE]);
 ...
 */
__ROM_USED void rt_timer_enter_sethook(void (*hook)(struct rt_timer *timer))
{
 ...
 * @param hook the hook function
 */
__ROM_USED void rt_timer_exit_sethook(void (*hook)(struct rt_timer *timer))
{
...
/**@}*/
        ...
        //static void rt_system_timer_recovery(uint8_t *user_data);
...
static struct rt_timer *rt_timer_list_next_timer(rt_list_t *timer_list)
{
    ...
    timer = rt_list_entry(timer_list->next,
                          struct rt_timer, row[RT_TIMER_SKIP_LIST_LEVEL - 1]);
...
__ROM_USED void rt_timer_dump(rt_list_t timer_heads[])
{
    rt_list_t *list;
 ...
 */
__ROM_USED void rt_timer_init(rt_timer_t  timer,
                              const char *name,
                              void (*timeout)(void *parameter),
                              void       *parameter,
                              rt_tick_t   time,
 ...
 */
__ROM_USED rt_err_t rt_timer_detach(rt_timer_t timer)
{
    register rt_base_t level;
 ...
 */
__ROM_USED rt_timer_t rt_timer_create(const char *name,
                                      void (*timeout)(void *parameter),
                                      void       *parameter,
                                      rt_tick_t   time,
                                      rt_uint8_t  flag)
{
    ...
    // Just for one list
    rt_list_t *list = &rt_timer_list[0];
    rt_timer_t timer_1 = rt_timer_list_next_timer(list);
    while (timer_1 && list->next != &rt_timer_list[0])
    {
        if (timer_1->parameter == parameter &&
                rt_strncmp(name, timer_1->parent.name, RT_NAME_MAX) == 0)
        {
            while (1);
        }
        list = list->next;
        timer_1 = rt_timer_list_next_timer(list);
    }
 ...
 * @return the operation status, RT_EOK on OK; RT_ERROR on error
 */
__ROM_USED rt_err_t rt_timer_delete(rt_timer_t timer)
{
    register rt_base_t level;
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 */
__ROM_USED rt_err_t rt_timer_start(rt_timer_t timer)
{
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 */
__ROM_USED rt_err_t rt_timer_stop(rt_timer_t timer)
{
    register rt_base_t level;
 ...
 * @return RT_EOK
 */
__ROM_USED rt_err_t rt_timer_control(rt_timer_t timer, int cmd, void *arg)
{
    register rt_base_t level;
 ...
 */
__ROM_USED void rt_timer_check(void)
{
    struct rt_timer *t;
...
__ROM_USED struct rt_timer *rt_timer_next_timer(void)
{
    rt_timer_t next_timer;

    next_timer = rt_timer_list_next_timer(&rt_timer_list[RT_TIMER_SKIP_LIST_LEVEL - 1]);

#ifdef RT_USING_TIMER_SOFT

    if (next_timer
            && (0 == rt_strncmp(next_timer->parent.name, "timer", RT_NAME_MAX)))
    {
        next_timer = rt_timer_list_next_timer(&rt_soft_timer_list[RT_TIMER_SKIP_LIST_LEVEL - 1]);
    }
 ...
 * corresponding timeout function will be invoked.
 */
__ROM_USED void rt_soft_timer_check(void)
{
    rt_tick_t current_tick;
    struct rt_timer *t;
    register rt_base_t level;
 ...
 */
__ROM_USED void rt_system_timer_init(void)
{
    int i;
 ...
 */
__ROM_USED void rt_system_timer_thread_init(void)
{
#ifdef RT_USING_TIMER_SOFT
    int i;
...
#ifdef RT_USING_PM
#ifdef PM_STANDBY_ENABLE
    //if (rt_application_get_power_on_mode() == 0)
    //deep_sleep_register(rt_system_timer_recovery, NULL);
#endif // PM_STANDBY_MODE
#endif // RT_USING_PM
}
