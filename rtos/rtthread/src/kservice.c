...
static rt_bool_t log_paused;
static const char *rt_version_str;
 ...
 * @return errno
 */
__ROM_USED rt_err_t rt_get_errno(void)
{
    rt_thread_t tid;
 ...
 */
__ROM_USED void rt_set_errno(rt_err_t error)
{
    rt_thread_t tid;
 ...
 */
__ROM_USED void *rt_memset(void *s, int c, rt_ubase_t count)
{
 ...
 */
__ROM_USED void *rt_memcpy(void *dst, const void *src, rt_ubase_t count)
{
#ifdef RT_USING_TINY_SIZE
 ...
 * @return the address of destination memory
 */
__ROM_USED void *rt_memmove(void *dest, const void *src, rt_ubase_t n)
{
 ...
 * @return the result
 */
__ROM_USED rt_int32_t rt_memcmp(const void *cs, const void *ct, rt_ubase_t count)
{
 ...
 */
__ROM_USED char *rt_strstr(const char *s1, const char *s2)
{
    int l1, l2;
 ...
 * @return the result
 */
__ROM_USED rt_uint32_t rt_strcasecmp(const char *a, const char *b)
{
    int ca, cb;
 ...
 * @return the result
 */
__ROM_USED char *rt_strncpy(char *dst, const char *src, rt_ubase_t n)
{
    if (n != 0)
    {
        char *d = dst;
 ...
 * @return the result
 */
__ROM_USED rt_int32_t rt_strncmp(const char *cs, const char *ct, rt_ubase_t count)
{
 ...
 * @return the result
 */
__ROM_USED rt_int32_t rt_strcmp(const char *cs, const char *ct)
{
    while (*cs && *cs == *ct)
        cs++, ct++;
 ...
 */
__ROM_USED rt_size_t rt_strnlen(const char *s, rt_ubase_t maxlen)
{
    const char *sc;
 ...
 * @return the length of string
 */
__ROM_USED rt_size_t rt_strlen(const char *s)
{
    const char *sc;
 ...
 */
__ROM_USED char *rt_strdup(const char *s)
{
...
#endif
__ROM_USED void rt_show_version(void)
{
#define str_(s)  #s
#define str(s)   str_(s)
    rt_kprintf("\n \\ | /\n");
    rt_kprintf("- SiFli Corporation\n");
    rt_kprintf(" / | \\     build on %s, %d.%d.%d build %s\n", __DATE__,
               SIFLI_VERSION >> 24, (SIFLI_VERSION & 0xFF0000) >> 16, SIFLI_VERSION & 0xFFFF,
               str(SIFLI_BUILD));
    rt_kprintf(" 2020 - 2022 Copyright by SiFli team\n");
}
RTM_EXPORT(rt_show_version);

__ROM_USED void rt_set_version(const char *ver)
{
    rt_version_str = ver;
}

__ROM_USED const char *rt_get_version(void)
{
    return rt_version_str;
}

__ROM_USED void rt_show_sys_info(void)
{
#if defined(RT_USING_FINSH) && !defined(CFG_BOOTLOADER)
    {
...
static char *cvt(double arg, int ndigits, int *decpt, int *sign, char *buf, int eflag)
{
    ...
    *sign = 0;
    ...
    {
        p1 = &buf[CVTBUFSIZE];
        while ((fi != 0) && (p < p1))//Avoid overwrite stack if arg is NaN/-INF/+INF on simulator
        {
            fj = modf(fi / 10, &fi);
            *--p1 = (int)((fj + .03) * 10) + '0';
            r2++;
 ...
 */
__ROM_USED rt_int32_t rt_snprintf(char *buf, rt_size_t size, const char *fmt, ...)
{
    rt_int32_t n;
    va_list args;
 ...
 */
__ROM_USED rt_int32_t rt_vsprintf(char *buf, const char *format, va_list arg_ptr)
{
 ...
 * @param format the format
 */
__ROM_USED rt_int32_t rt_sprintf(char *buf, const char *format, ...)
{
    rt_int32_t n;
    va_list arg_ptr;
 ...
 */
__ROM_USED rt_device_t rt_console_get_device(void)
{
 ...
 */
__ROM_USED rt_device_t rt_console_set_device(const char *name)
{
    rt_device_t new, old;
    ...
    if (new != RT_NULL)
    {
        ...
        {
            rt_uint16_t oflag = RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_STREAM;
            if (new->flag & RT_DEVICE_FLAG_DMA_RX)
            {
                oflag |= RT_DEVICE_FLAG_DMA_RX;
            }
            if (new->flag & RT_DEVICE_FLAG_DMA_TX)
            {
                oflag |= RT_DEVICE_FLAG_DMA_TX;
            }
            rt_device_open(new, oflag);
        }
        _console_device = new;
    }
 ...
 * @param fmt the format
 */
__ROM_USED void rt_kprintf(const char *fmt, ...)
{
    va_list args;
    rt_size_t length;
...
#ifndef RT_HEXDUMP_LINE_BUF_SIZE
    #define RT_HEXDUMP_LINE_BUF_SIZE 128
#endif /* RT_HEXDUMP_LINE_BUF_SIZE */

#ifndef RT_HEXDUMP_NEWLINE_SIGN
    #define RT_HEXDUMP_NEWLINE_SIGN              "\r\n"
#endif

static size_t rt_hexdump_strcpy(size_t cur_len, char *dst, const char *src)
{
    ...
    while (*src != 0)
    {
        ...
        if (cur_len++ < RT_HEXDUMP_LINE_BUF_SIZE)
        {
            *dst++ = *src++;
        }
        else
...
#define __is_print(ch)       ((unsigned int)((ch) - ' ') < 127u - ' ')
    ...
    int fmt_result;
    char line_buf[RT_HEXDUMP_LINE_BUF_SIZE + 1 /*for '\0' */];
    ...
    for (i = 0, log_len = 0; i < size; i += width)
    {
        ...
        if (i == 0)
        {
            log_len += rt_hexdump_strcpy(log_len, log_buf + log_len, "D/HEX ");
            log_len += rt_hexdump_strcpy(log_len, log_buf + log_len, tag);
            log_len += rt_hexdump_strcpy(log_len, log_buf + log_len, ": ");
        }
        else
        {
        ...
        RT_ASSERT(log_len < RT_HEXDUMP_LINE_BUF_SIZE);
        fmt_result = rt_snprintf(log_buf + log_len, RT_HEXDUMP_LINE_BUF_SIZE - log_len, "%04X-%04X: ", i, i + width);
        /* calculate log length */
        if ((fmt_result > -1) && (fmt_result <= (int)(RT_HEXDUMP_LINE_BUF_SIZE - log_len)))
        {
            log_len += fmt_result;
        }
        else
        {
            log_len = RT_HEXDUMP_LINE_BUF_SIZE;
        }
        /* dump hex */
        for (j = 0; j < width; j++)
        {
            ...
            log_len += rt_hexdump_strcpy(log_len, log_buf + log_len, dump_string);
            if ((j + 1) % 8 == 0)
            {
                log_len += rt_hexdump_strcpy(log_len, log_buf + log_len, " ");
            }
        ...
        if (log_len + rt_strlen(RT_HEXDUMP_NEWLINE_SIGN) > RT_HEXDUMP_LINE_BUF_SIZE)
        {
            log_len = RT_HEXDUMP_LINE_BUF_SIZE - rt_strlen(RT_HEXDUMP_NEWLINE_SIGN);
        }
        /* package newline sign */
        log_len += rt_hexdump_strcpy(log_len, log_buf + log_len, RT_HEXDUMP_NEWLINE_SIGN);
...
void log_pause(rt_bool_t pause)
{
    if (pause == log_paused)
    {
        return;
 ...
 */
__ROM_USED void *rt_malloc_align(rt_size_t size, rt_size_t align)
{
    void *align_ptr;
    void *ptr;
 ...
 */
__ROM_USED void rt_free_align(void *ptr)
{
    void *real_ptr;
 ...
 * shall return 0.
 */
__ROM_USED int __rt_ffs(int value)
{
    if (value == 0) return 0;
...
#ifdef RT_DEBUG
/* RT_ASSERT(EX)'s hook */
__ROM_USED void (*rt_assert_hook)(const char *ex, const char *func, rt_size_t line);
/**
 * This function will set a hook function to RT_ASSERT(EX). It will run when the expression is false.
 *
 * @param hook the hook function
 */
__ROM_USED void rt_assert_set_hook(void (*hook)(const char *ex, const char *func, rt_size_t line))
{
    rt_assert_hook = hook;
}

#if defined(_MSC_VER)
RT_WEAK void rt_hw_fatal_error(void)
{
    volatile rt_uint32_t a = 1234;
    volatile rt_uint32_t d = 0;

    if (a / d > 1) //Raise 'division by zero' error
    {
        while (1) {};
    }
 ...
 */
__ROM_USED void rt_assert_handler(const char *ex_string, const char *func, rt_size_t line)
{
    volatile char dummy = 0;
    rt_ubase_t level;
    static rt_bool_t assert_happened = RT_FALSE;

    {
...
#endif
        {
            ...
            //Save previous level before disbale it.
            level = rt_hw_interrupt_disable();

            if (RT_FALSE == assert_happened)
            {
                assert_happened = RT_TRUE;
            }
            else
            {
                /* Don't trigger assertion if it already happens.
                 * But hardfault could still happen as hardfault interrupt cannot be disabled
                 *
                 */
                while (1);
            }
    ...
    /* Not use while(1) as it breaks backtrace */
