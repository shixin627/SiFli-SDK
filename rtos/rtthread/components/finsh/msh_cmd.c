...
int cmd_mountfs(int argc, char **argv)
{
    int result = 0;
    char *type = "elm"; /* use the default file system type as 'fatfs' */
    ...
    if (argc == 3)
    {
        result = dfs_mount(argv[1], argv[2], "elm", 0, 0);
    }
    else if (argc == 5)
    {
        if (strcmp(argv[1], "-t") == 0)
        {
            type = argv[2];
            result = dfs_mount(argv[3], argv[4], type, 0, 0);
        }
    }
    else
    {
        rt_kprintf("Usage: mountfs [-t type] device dir\n");
        return 0;
    }

    if (result != RT_EOK)
    {
        rt_kprintf("mountfs failed, result=%d\n", result);
    }
