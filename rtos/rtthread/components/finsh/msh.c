...
__ROM_USED rt_bool_t msh_is_used(void)
{
    return RT_TRUE;
}
#else
...
__ROM_USED void msh_set_console(const char *name)
{
    rt_console_set_device(name);
#ifdef RT_USING_POSIX
    libc_stdio_set_console(name, O_RDWR);
#else
    finsh_set_device(name);
#endif
}

__ROM_USED int msh_console(int argc, char **argv)
{
    if (argc >= 2)
    {
        msh_set_console(argv[1]);
    }
    return 0;
}
FINSH_FUNCTION_EXPORT_ALIAS(msh_console, __cmd_console, Change MSH / FINSH console device.);
#endif

__ROM_USED int msh_help(int argc, char **argv)
{
...
__ROM_USED void msh_auto_complete(char *prefix)
{
