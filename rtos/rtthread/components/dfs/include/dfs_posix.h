...
#ifdef WIN32
// MSVC could not replace system CRT funcitons. Use new name in Simulator for Python file access.

int rt_open(const char *file, int flags, ...);
int rt_close(int d);
int rt_read(int fd, void *buf, size_t len);
int rt_write(int fd, const void *buf, size_t len);
off_t rt_lseek(int fd, off_t offset, int whence);
int rt_fstat(int fildes, struct stat *buf);
#endif
