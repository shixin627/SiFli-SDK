                    ...
                    uint8_t *page_buf, uint8_t gc_ratio)
{
    ...
    cap = dhara_journal_capacity(&m->journal);
    reserve = cap / (m->gc_ratio + 1);
    if (m->gc_ratio < 5)    /* keep default setting */
        m->gc_cnt = m->gc_ratio;
    else
    {
        m->gc_cnt = cap / (reserve - (1 << m->journal.log2_ppc)) + 1;
        if (m->gc_cnt >= 1)
            m->gc_cnt -= 1;
    }

    if (m->gc_cnt < m->gc_ratio)
        m->gc_cnt = m->gc_ratio;
    else if (m->gc_cnt > 2 * m->gc_ratio)
        m->gc_cnt = 2 * m->gc_ratio;
...
#else
    const dhara_sector_t safety_margin = 0;
                  ...
                  dhara_error_t *err)
{
    ...
    {
        if (DHARA_E_ECC == *err)
            /* Corrupted checkpoint may be caused by power-loss */
            return 0;
        else
            return -1;
    }
...
int dhara_map_gc_all(struct dhara_map *m, dhara_error_t *err)
{
    int r;

    while (dhara_journal_size(&m->journal) > m->count)
    {
        r = dhara_map_gc(m, err);
        if (r < 0)
        {
            return r;
        }
...
int dhara_map_get_type(struct dhara_map *m, dhara_page_t p)
{
    dhara_error_t err;
    dhara_sector_t target;
    uint8_t meta[DHARA_META_SIZE];
     ...
     * currently resides (if anywhere).
     */
    if (trace_path(m, target, &current, meta, &err) < 0)
    {
