...
static rt_err_t rt_serial_close(struct rt_device *dev)
{
    ...
    if (dev->open_flag & RT_DEVICE_FLAG_INT_RX)
    {
        ...
        dev->open_flag &= ~RT_DEVICE_FLAG_INT_RX;
        RT_ASSERT(serial->serial_rx != NULL);
    ...
    if (dev->open_flag & RT_DEVICE_FLAG_INT_TX)
    {
        ...
        dev->open_flag &= ~RT_DEVICE_FLAG_INT_TX;
        RT_ASSERT(serial->serial_tx != NULL);
 ...
 * serial register
 */
__ROM_USED rt_err_t rt_hw_serial_register(struct rt_serial_device *serial,
        const char              *name,
        rt_uint32_t              flag,
        void                    *data)
{
...
__ROM_USED void rt_hw_serial_isr(struct rt_serial_device *serial, int event)
{
    switch (event & 0xff)
    {
