                               ...
                               rt_size_t   size)
{
...
#endif
    {
        ...
        if (rt_ringbuffer_space_len(&(pipe->ringbuffer)) < size)
        {
            g_pipe_full_cnt++;
            if (g_pipe_full_cnt == 1)
 ...
 */
__ROM_USED rt_err_t rt_audio_pipe_init(struct rt_audio_pipe *pipe,
                                       const char *name,
                                       rt_int32_t flag,
                                       rt_uint8_t *buf,
                                       rt_size_t size)
 ...
 * @return the operation status, RT_EOK on successful
 */
__ROM_USED rt_err_t rt_audio_pipe_detach(struct rt_audio_pipe *pipe)
{
    return rt_device_unregister(&pipe->parent);
}

#ifdef RT_USING_HEAP
__ROM_USED rt_err_t rt_audio_pipe_create(const char *name, rt_int32_t flag, rt_size_t size)
{
...
__ROM_USED void rt_audio_pipe_destroy(struct rt_audio_pipe *pipe)
{
    if (pipe == RT_NULL)
        return;
    ...
    /* release memory */
    rt_free(pipe->ringbuffer.wr_buffer_ptr);
    rt_free(pipe);
