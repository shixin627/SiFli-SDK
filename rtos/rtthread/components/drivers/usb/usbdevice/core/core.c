...
static rt_err_t _set_address(struct udevice *device, ureq_t setup)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_USB, ("_set_address 0x%x\n", setup->wValue));
...
static rt_err_t _function_request(udevice_t device, ureq_t setup)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_USB, ("%s %d setup->wIndex=0x%x bNumInterfaces=0x%x\n", __func__, __LINE__, setup->wIndex, device->curr_cfg->cfg_desc.bNumInterfaces));
    if (0xa1 == setup->request_type)
    {
        if (0x100 < setup->wIndex)
        {
            intf = rt_usbd_find_interface(device, setup->wIndex >> 8, &func);
            RT_DEBUG_LOG(RT_DEBUG_USB, ("%s %d intf=%p\n", __func__, __LINE__, intf));
...
static rt_err_t _vendor_request(udevice_t device, ureq_t setup)
{
    ...
    case 'A':
        ...
        case 0x04:
            ...
            {
                rt_usbd_ep0_set_stall(device);
                RT_DEBUG_LOG(RT_DEBUG_USB, ("rt_usbd_ep0_set_stall\n"));
                return RT_EOK;
            }
        ...
        case 0x05:
            ...
            {
                RT_DEBUG_LOG(RT_DEBUG_USB, ("rt_usbd_find_interface fail\n"));
            }
            break;
        }
...
static rt_size_t rt_usbd_ep_write(udevice_t device, uep_t ep, void *buffer, rt_size_t size)
{
    ...
    {
        RT_DEBUG_LOG(RT_DEBUG_USB, ("%s %d,buffer=%s\n", __func__, __LINE__, buffer));
 ...
 * @return RT_EOK.
 */
static const rt_uint8_t test_data[] =  // Total 53 bytes
{
...
rt_err_t rt_usbd_set_feature(udevice_t device, rt_uint16_t value, rt_uint16_t index)
{
    ...
    {
        RT_DEBUG_LOG(RT_DEBUG_USB, ("test mode\n"));
        dcd_test_mode(device->dcd, index, (uint8_t *)test_data, sizeof(test_data));
    }
...
static void rt_usbd_thread_entry(void *parameter)
{
    while (1)
    {
        ...
        case USB_MSG_PLUG_IN:
            RT_DEBUG_LOG(RT_DEBUG_USB, ("usb plug in\n"));
            device->state = USB_STATE_ATTACHED;
            break;
        case USB_MSG_PLUG_OUT:
            RT_DEBUG_LOG(RT_DEBUG_USB, ("usb plug out\n"));
            device->state = USB_STATE_NOTATTACHED;
            _stop_notify(device);
            break;
        default:
...
L1_NON_RET_BSS_SECT_BEGIN(usb_thread_stack)
ALIGN(RT_ALIGN_SIZE)
L1_NON_RET_BSS_SECT(usb_thread_stack, static rt_uint8_t usb_thread_stack[RT_USBD_THREAD_STACK_SZ]);
...
ALIGN(RT_ALIGN_SIZE) static rt_uint8_t usb_mq_pool[(USBD_MQ_MSG_SZ + sizeof(void *))*USBD_MQ_MAX_MSG];
