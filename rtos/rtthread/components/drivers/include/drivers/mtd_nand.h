...
struct rt_mtd_nand_device
{
    ...
    void *cache_buf;                 /* Alloc 128KB cache for write and erase */
    rt_uint32_t blk_cnum;           /* Block number in write cache */
...
struct rt_mtd_nand_driver_ops
{
    ...
    rt_err_t (*read_page_with_offset)(struct rt_mtd_nand_device *device,
...
rt_inline rt_err_t rt_mtd_nand_read_with_offset(
    struct rt_mtd_nand_device *device,
    rt_off_t page,  rt_uint32_t offset,
    rt_uint8_t *data, rt_uint32_t data_len,
    rt_uint8_t *spare, rt_uint32_t spare_len)
