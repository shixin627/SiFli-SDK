...
#define AUDIO_CTL_SETOUTPUT                 _AUDIO_CTL(12)
#define AUDIO_CTL_SETINPUT                  _AUDIO_CTL(13)
#define AUDIO_CTL_OUTPUTSRC                 _AUDIO_CTL(14)
#define AUDIO_CTL_INPUTSRC                  _AUDIO_CTL(15)
#define AUDIO_CTL_SETVOLUME                 _AUDIO_CTL(16)
#define AUDIO_CTL_MUTE                      _AUDIO_CTL(17)
#define AUDIO_CTL_SET_TX_DMA_SIZE           _AUDIO_CTL(18)
...
#define AUDIO_TYPE_LOOPBACK                 0x20
...
#define CFG_AUDIO_REPLAY_QUEUE_COUNT        2
#if defined(PKG_USING_SOUNDPLUS) || defined(PKG_USING_3MICS)
    #define CFG_AUDIO_RECORD_PIPE_SIZE          (480)
#else
    #define CFG_AUDIO_RECORD_PIPE_SIZE          (320)
#endif
#define AUDIO_DEVICE_MP_CNT                 (4)
//#define AUDIO_DEVICE_DECODE_MP_BLOCK_SZ     (4352 * 4)
#define AUDIO_DEVICE_DECODE_MP_BLOCK_SZ     (64 * 4)
...
enum
{
    ...
    AUDIO_STREAM_RECORD,
    AUDIO_STREAM_RXandTX,  //only for audprc
    AUDIO_STREAM_PDM_PRESTART, // only for pmd start 3mics
    AUDIO_STREAM_PDM_START, // only for pmd start 3mics
    AUDIO_STREAM_LAST = AUDIO_STREAM_PDM_START,
};
...
{
    rt_bool_t activated;
    struct rt_audio_pipe *audio_pipe;
};
...
struct rt_audio_sr_convert
{
    rt_uint32_t channel;

    rt_uint32_t source_sr;
    rt_uint32_t dest_sr;
};
