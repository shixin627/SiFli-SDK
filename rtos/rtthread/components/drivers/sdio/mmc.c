                      ...
                      rt_uint8_t index, rt_uint8_t value)
{
    ...
    cmd.flags = RESP_SPI_R1 | RESP_R1B | CMD_AC;
...
static int mmc_select_bus_width(struct rt_mmcsd_card *card, rt_uint8_t *ext_csd)
{
    ...
    {
#if (SD_MMC_DDR_SUPPORT == 1)
#if (SD_MMC_OLINE_SUPPORT == 1)
...
#if  (SD_MMC_OLINE_SUPPORT == 1)
        EXT_CSD_BUS_WIDTH_8,
#endif  //(SD_MMC_OLINE_SUPPORT == 1)
#endif  //(SD_MMC_DDR_SUPPORT == 1)
        EXT_CSD_BUS_WIDTH_4,
        EXT_CSD_BUS_WIDTH_1
    };
    ...
    */
    for (idx = 0; idx < sizeof(bus_widths) / sizeof(rt_uint32_t); idx++)
    {
        ...
        */
        if (ext_csd_bits[idx] == EXT_CSD_DDR_BUS_WIDTH_4 || ext_csd_bits[idx] == EXT_CSD_DDR_BUS_WIDTH_8)
        {
        ...
        */
        ...
        else
        {
            ...
            case 2:
                ...
                break;
            case 5:
                LOG_E("switch to bus ddr 4 bit failed!");
                break;
            case 6:
                LOG_E("switch to bus ddr 8 bit failed!");
                break;
            default:
                break;
            }
