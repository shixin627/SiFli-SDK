 ...
 * 2021-07-20     arminker     fix write_index bug in function rt_ringbuffer32_put_force
...
rt_inline enum rt_ringbuffer32_state rt_ringbuffer32_status(struct rt_ringbuffer32 *rb)
{
    if (rb->read_index == rb->write_index)
    {
        if (rb->read_mirror == rb->write_mirror)
            return RT_RINGBUFFER32_EMPTY;
        else
            return RT_RINGBUFFER32_FULL;
    }
    return RT_RINGBUFFER32_HALFFULL;
}
 ...
 */
void rt_ringbuffer32_init(struct rt_ringbuffer32 *rb,
                          rt_uint8_t           *pool,
                          rt_int32_t            size)
{
...
RTM_EXPORT(rt_ringbuffer32_init);
 ...
 */
rt_size_t rt_ringbuffer32_put(struct rt_ringbuffer32 *rb,
                              ...
                              rt_uint32_t           length)
{
    ...
    size = rt_ringbuffer32_space_len(rb);
...
RTM_EXPORT(rt_ringbuffer32_put);
 ...
 * @return Return the data size we put into the ring buffer.
 */
rt_size_t rt_ringbuffer32_put_force(struct rt_ringbuffer32 *rb,
                                    const rt_uint8_t     *ptr,
                                    rt_uint32_t           length)
{
    ...
    space_length = rt_ringbuffer32_space_len(rb);
...
RTM_EXPORT(rt_ringbuffer32_put_force);
 ...
 */
rt_size_t rt_ringbuffer32_get(struct rt_ringbuffer32 *rb,
                              rt_uint8_t           *ptr,
                              rt_uint32_t           length)
{
    rt_size_t size;
    ...
    size = rt_ringbuffer32_data_len(rb);
...
RTM_EXPORT(rt_ringbuffer32_get);
 ...
 */
rt_size_t rt_ringbuffer32_get_direct(struct rt_ringbuffer32 *rb, rt_uint8_t **ptr)
{
    rt_size_t size;
...
RTM_EXPORT(rt_ringbuffer32_get_direct);
 ...
 */
rt_size_t rt_ringbuffer32_peek(struct rt_ringbuffer32 *rb, rt_uint8_t **ptr)
{
    rt_size_t size;
...
RTM_EXPORT(rt_ringbuffer32_peek);
 ...
 */
rt_size_t rt_ringbuffer32_putchar(struct rt_ringbuffer32 *rb, const rt_uint8_t ch)
{
    RT_ASSERT(rb != RT_NULL);

    /* whether has enough space */
    if (!rt_ringbuffer32_space_len(rb))
        return 0;
...
RTM_EXPORT(rt_ringbuffer32_putchar);
 ...
 */
rt_size_t rt_ringbuffer32_putchar_force(struct rt_ringbuffer32 *rb, const rt_uint8_t ch)
{
    enum rt_ringbuffer32_state old_state;

    RT_ASSERT(rb != RT_NULL);

    old_state = rt_ringbuffer32_status(rb);
    ...
    if (rb->write_index == rb->buffer_size - 1)
    {
        rb->write_mirror = ~rb->write_mirror;
        rb->write_index = 0;
        if (old_state == RT_RINGBUFFER32_FULL)
        {
            rb->read_mirror = ~rb->read_mirror;
            rb->read_index = rb->write_index;
        }
...
RTM_EXPORT(rt_ringbuffer32_putchar_force);
 ...
 * @return 1    Success
 */
rt_size_t rt_ringbuffer32_getchar(struct rt_ringbuffer32 *rb, rt_uint8_t *ch)
{
    RT_ASSERT(rb != RT_NULL);

    /* ringbuffer is empty */
    if (!rt_ringbuffer32_data_len(rb))
        return 0;
...
RTM_EXPORT(rt_ringbuffer32_getchar);
 ...
 */
rt_size_t rt_ringbuffer32_data_len(struct rt_ringbuffer32 *rb)
{
    switch (rt_ringbuffer32_status(rb))
    {
    case RT_RINGBUFFER32_EMPTY:
        return 0;
    case RT_RINGBUFFER32_FULL:
        return rb->buffer_size;
    case RT_RINGBUFFER32_HALFFULL:
    default:
    {
...
RTM_EXPORT(rt_ringbuffer32_data_len);
 ...
 * @param rb        A pointer to the ring buffer object.
 */
void rt_ringbuffer32_reset(struct rt_ringbuffer32 *rb)
{
    RT_ASSERT(rb != RT_NULL);
...
RTM_EXPORT(rt_ringbuffer32_reset);
 ...
 */
struct rt_ringbuffer32 *rt_ringbuffer32_create(rt_uint32_t size)
{
    ...
    rb = (struct rt_ringbuffer32 *)rt_malloc(sizeof(struct rt_ringbuffer32));
    if (rb == RT_NULL)
        goto exit;
    ...
    rt_ringbuffer32_init(rb, pool, size);

exit:
    return rb;
}
RTM_EXPORT(rt_ringbuffer32_create);

/**
 * @brief Destroy the ring buffer object, which is created by rt_ringbuffer32_create() .
 *
 * @param rb        A pointer to the ring buffer object.
 */
void rt_ringbuffer32_destroy(struct rt_ringbuffer32 *rb)
{
    RT_ASSERT(rb != RT_NULL);
...
RTM_EXPORT(rt_ringbuffer32_destroy);
