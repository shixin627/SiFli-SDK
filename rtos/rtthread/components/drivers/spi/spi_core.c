...
extern rt_err_t rt_spidev_device_init(struct rt_spi_device *dev, const char *name, rt_uint32_t flag);

__ROM_USED rt_err_t rt_spi_bus_register(struct rt_spi_bus       *bus,
                                        const char              *name,
                                        rt_uint32_t              flag,
...
__ROM_USED rt_err_t rt_spi_bus_attach_device(struct rt_spi_device *device,
        const char           *name,
        const char           *bus_name,
        void                 *user_data)
{
    ...
    if (bus != RT_NULL && bus->type == RT_Device_Class_SPIBUS)
    {
        ...
        result = rt_spidev_device_init(device, name, bus->flag & RT_DEVICE_OFLAG_MASK);
        if (result != RT_EOK)
            return result;
...
__ROM_USED rt_err_t rt_spi_configure(struct rt_spi_device        *device,
                                     struct rt_spi_configuration *cfg)
{
    ...
    device->config.frameMode  = cfg->frameMode ;
...
__ROM_USED rt_err_t rt_spi_send_then_send(struct rt_spi_device *device,
...
__ROM_USED rt_err_t rt_spi_send_then_recv(struct rt_spi_device *device,
        const void           *send_buf,
...
__ROM_USED rt_size_t rt_spi_transfer(struct rt_spi_device *device,
                                     const void           *send_buf,
                                     void                 *recv_buf,
                                     rt_size_t             length)
{
...
__ROM_USED struct rt_spi_message *rt_spi_transfer_message(struct rt_spi_device  *device,
        struct rt_spi_message *message)
{
    rt_err_t result;
...
__ROM_USED rt_err_t rt_spi_release_bus(struct rt_spi_device *device)
{
    RT_ASSERT(device != RT_NULL);
    RT_ASSERT(device->bus != RT_NULL);
...
__ROM_USED rt_err_t rt_spi_take(struct rt_spi_device *device)
{
    rt_err_t result;
    struct rt_spi_message message;
...
__ROM_USED rt_err_t rt_spi_release(struct rt_spi_device *device)
{
    rt_err_t result;
    struct rt_spi_message message;
