 ...
 *
 */
__ROM_USED rt_err_t set_date(rt_uint32_t year, rt_uint32_t month, rt_uint32_t day)
{
    time_t now;
    struct tm *p_tm;
    struct tm tm_new;
 ...
 *
 */
__ROM_USED rt_err_t set_time(rt_uint32_t hour, rt_uint32_t minute, rt_uint32_t second)
{
    time_t now;
    struct tm *p_tm;
    struct tm tm_new;
...
/**
  ...
  * @retval HAL status
  */
__ROM_USED rt_err_t add_1s(rt_int32_t increase)
{
    ...
    return rt_device_control(device, RT_DEVICE_CTRL_RTC_INC_1S, (void *)increase);
#else
    return - RT_ERROR;            // Not supported.
#endif
}
...
__ROM_USED void list_date(void)
{
    time_t now;
...
#if defined(RT_USING_FINSH) && defined(FINSH_USING_MSH) && !defined(LCPU_MEM_OPTIMIZE)
__ROM_USED void show_date(uint8_t argc, char **argv)
{
    if (argc == 1)
    {
        time_t now;
    ...
    else if (argc >= 7)
    {
        ...
        {
            rt_kprintf("year is out of range [1970-2099]\n");
            return;
        }
        if (month == 0 || month > 12)
        {
...
MSH_CMD_EXPORT(show_date, get date and time or set [year month day hour min sec]);
