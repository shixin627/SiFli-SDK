...
/* alarm thread */
    ...
    ALIGN(RT_ALIGN_SIZE)
    L1_NON_RET_BSS_SECT(alarm_thread_stack, static char alarm_thread_stack[ALARM_THREAD_STACK_SIZE]);
 ...
 * \return none
 */
__ROM_USED void rt_alarm_update(rt_device_t dev, rt_uint32_t event)
{
 ...
 * \param arg argument
 */
__ROM_USED rt_err_t rt_alarm_control(rt_alarm_t alarm, int cmd, void *arg)
{
    rt_err_t ret = RT_ERROR;
 ...
 * \return RT_EOK
 */
__ROM_USED rt_err_t rt_alarm_start(rt_alarm_t alarm)
{
 ...
 */
__ROM_USED rt_alarm_t rt_alarm_create(rt_alarm_callback_t callback, struct rt_alarm_setup *setup)
{
    struct rt_alarm *alarm;
    ...
    alarm = LLT_MEM_ALLOC(sizeof(struct rt_alarm));
    if (alarm == RT_NULL)
        return (RT_NULL);
 ...
 * \return none
 */
__ROM_USED int rt_alarm_system_init(void)
{
    rt_thread_t tid;
    rt_err_t result;
    ...
    tid = rt_thread_create("alarmsvc",
                           ...
                           ALARM_THREAD_STACK_SIZE, RT_THREAD_PRIORITY_HIGH, RT_THREAD_TICK_DEFAULT / 2);
#else
    tid = &alarm_thread;
    result = rt_thread_init(tid, "alarmsvc", rt_alarmsvc_thread_init, RT_NULL,
                            alarm_thread_stack, sizeof(alarm_thread_stack), RT_THREAD_PRIORITY_HIGH, RT_THREAD_TICK_DEFAULT / 2);

#endif

    if ((tid != RT_NULL) && (RT_EOK == result))
