...
/**
  * @brief  ILI9327 Timing
  */
/* Timing configuration  (Typical configuration from ILI9327 datasheet)
  HSYNC=10 (9+1)
  HBP=20 (29-10+1)
  ...
  VFP=4 (327-320-2-2+1)
*/
#define  ILI9327_HSYNC            ((uint32_t)9)   /* Horizontal synchronization */
#define  ILI9327_HBP              ((uint32_t)29)    /* Horizontal back porch      */
#define  ILI9327_HFP              ((uint32_t)2)    /* Horizontal front porch     */
#define  ILI9327_VSYNC            ((uint32_t)1)   /* Vertical synchronization   */
#define  ILI9327_VBP              ((uint32_t)3)    /* Vertical back porch        */
#define  ILI9327_VFP              ((uint32_t)2)    /* Vertical front porch       */

/**
  * @brief  ILI9327 Registers
  */
...
static void LCD_Init(LCDC_HandleTypeDef *hlcdc)
{
    ...
    /* Initialize ILI9327 low level bus layer ----------------------------------*/
    ...
    parameter[1] = 0x62;
    parameter[2] = 0x1F;
    LCD_WriteReg(hlcdc, 0xD1, parameter, 3);

    // power setting
    parameter[0] = 0x07;
    parameter[1] = 0x07;
    parameter[2] = 0x80;
    LCD_WriteReg(hlcdc, 0xD0, parameter, 3);
    ...
    parameter[4] = 0x01;
    parameter[5] = 0x02;
    LCD_WriteReg(hlcdc, 0xC0, parameter, 6);
...
// 3’h7 60
    ...
    // power settings for normal mode
    parameter[0] = 0x01;
    parameter[1] = 0x22;
    LCD_WriteReg(hlcdc, 0xD2, parameter, 2);

    /*--------------- ILI9327 Gamma setting ------------------------------------*/
    parameter[0] = 0x00;
    parameter[1] = 0x30;
    parameter[2] = 0x33;
    ...
    parameter[13] = 0x80;
    parameter[14] = 0x00;
    LCD_WriteReg(hlcdc, 0xC8, parameter, 15);
...
static uint32_t LCD_ReadPixel(LCDC_HandleTypeDef *hlcdc, uint16_t Xpos, uint16_t Ypos)
{
    ...
    */
    ...
    case LCDC_PIXEL_FORMAT_RGB666:
        /*
           pretend that ili9327 can support RGB666,
