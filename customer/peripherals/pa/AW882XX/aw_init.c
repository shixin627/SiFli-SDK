...
void aw882xx_interrupt_setup(struct aw_device *aw_dev)
{
    ...
    aw882xx_i2c_read(aw882xx, aw882xx->aw_pa->int_desc.mask_reg, &reg_val);
    reg_val &= (~aw882xx->aw_pa->int_desc.uvl_val);
    reg_val &= (~aw882xx->aw_pa->int_desc.clock_val);
    reg_val &= (~aw882xx->aw_pa->int_desc.clks_val);
    aw882xx_i2c_write(aw882xx, aw882xx->aw_pa->int_desc.mask_reg, reg_val);
}
...
void aw882xx_interrupt_clear(struct aw_device *aw_dev)
{
    ...
    aw882xx_i2c_read(aw882xx, aw882xx->aw_pa->sysst_desc.reg, &reg_val);
    aw_dev_info(aw882xx->dev_index, "reg SYSST=0x%x", reg_val);
    aw882xx_i2c_read(aw882xx, aw882xx->aw_pa->int_desc.st_reg, &reg_val);
    aw_dev_info(aw882xx->dev_index, "reg SYSINT=0x%x", reg_val);
    aw882xx_i2c_read(aw882xx, aw882xx->aw_pa->int_desc.mask_reg, &reg_val);
    aw_dev_info(aw882xx->dev_index, "reg SYSINTM=0x%x", reg_val);
}
#endif
                                ...
                                struct aw_fill_info *fill_info)
{
...
#ifdef AW_IRQ
    aw_pa->ops.aw882xx_interrupt_setup = aw882xx_interrupt_setup;
    aw_pa->ops.aw882xx_interrupt_clear = aw882xx_interrupt_clear;
#endif
    ...
    ret = aw_device_probe(aw_pa, fill_info);
...
static void aw_pid_2013_efver_check(struct aw_device *aw_dev)
{
    ...
    aw_dev_dbg(aw_dev->dev_index, "efverh: 0x%0x, efverl: 0x%0x", efverh, efverl);

    if (efverh && efverl)
    {
        aw_dev->bstcfg_enable = AW_BSTCFG_ENABLE;
        aw_dev_info(aw_dev->dev_index, "A2013 EFVER A");
    }
    else if (efverh && !efverl)
    {
        aw_dev->bstcfg_enable = AW_BSTCFG_DISABLE;
        aw_dev_info(aw_dev->dev_index, "A2013 EFVER B");
    }
    else
    {
        aw_dev->bstcfg_enable = AW_BSTCFG_DISABLE;
        aw_dev_info(aw_dev->dev_index, "unsupport A2013 EFVER");
    }
    aw_dev_info(aw_dev->dev_index, "bstcfg enable: %d", aw_dev->bstcfg_enable);
}

uint32_t i2ssr_tab[10] = {8000, 11000, 12000, 16000, 22000, 24000, 32000, 44000, 48000, 96000};

static int aw_pid_2013_set_sf(struct aw_device *aw_dev, uint32_t samplerate, uint8_t chn_sel)
{
    struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
    unsigned int reg_value = 0;
    unsigned int real_value;

    for (real_value = 0; real_value < 10; real_value++)
    {
        if (i2ssr_tab[real_value] == samplerate)
        {
            break;
        }
    }
    if (real_value == 10)
    {
        aw_dev_err(aw882xx->dev_index, "unsupported samplerate:%d", samplerate);
        return -1;
    }

    if ((chn_sel > 3) || (chn_sel < 1))
    {
        aw_dev_err(aw882xx->dev_index, "unsupported channel sel:%d", chn_sel);
        return -1;
    }

    /* cal real value */
    aw882xx_i2c_read(aw882xx, AW_PID_2013_I2SCTRL1_REG, &reg_value);

    /*[3 : 0] samplerate*/
    real_value = real_value | (reg_value & 0xfff0);

    /*[11 : 10] CHSEL*/
    real_value = (chn_sel << 10) | (real_value & 0xf3ff);

    aw_dev_err(aw882xx->dev_index, "value %d , chn_sel %d, 0x%x , 0x%x\n", samplerate, chn_sel, real_value, reg_value);

    /* write value */
    aw882xx_i2c_write(aw882xx, AW_PID_2013_I2SCTRL1_REG, real_value);
    return 0;
}
                                ...
                                struct aw_fill_info *fill_info)
{
    ...
    aw_pa->ops.aw882xx_interrupt_clear = aw882xx_interrupt_clear;
#endif
    aw_pa->ops.aw_set_sf = aw_pid_2013_set_sf;
    ...
    aw_pa->int_desc.uvl_val = AW_PID_2013_UVLS_UVLO_VALUE;
    aw_pa->int_desc.clock_val = AW_PID_2013_NOCLKS_CLOCK_OK_VALUE;
    aw_pa->int_desc.clks_val = AW_PID_2013_CLKS_STABLE_VALUE;
...
static int aw_pid_2032_set_volume(struct aw_device *aw_dev, unsigned int value)
{
    ...
    aw_dev_dbg(aw_dev->dev_index, "value %d , 0x%x", value, real_value);
                                ...
                                struct aw_fill_info *fill_info)
{
    ...
    aw_pa->int_desc.uvl_val = AW_PID_2032_UVLS_VDD_BELOW_2P8V_VALUE;
    aw_pa->int_desc.clock_val = AW_PID_2032_NOCLKS_CLOCK_OK_VALUE;
    aw_pa->int_desc.clks_val = AW_PID_2032_CLKS_STABLE_VALUE;
    ...
    aw_pa->profctrl_desc.reg = AW_PID_2032_SYSCTRL_REG;
    aw_pa->profctrl_desc.mask = AW_PID_2032_RCV_MODE_MASK;
    aw_pa->profctrl_desc.spk_mode = AW_PID_2032_RCV_MODE_SPEAKER_VALUE;
                                 ...
                                 struct aw_fill_info *fill_info)
{
    ...
    aw_pa->int_desc.uvl_val = AW_PID_2055A_UVLS_UVLO_VALUE;
    aw_pa->int_desc.clock_val = AW_PID_2055A_NOCLKS_CLOCK_OK_VALUE;
    aw_pa->int_desc.clks_val = AW_PID_2055A_CLKS_STABLE_VALUE;
    ...
    aw_pa->sysst_desc.pll_check = AW_PID_2055A_PLLS_LOCKED_VALUE;

    aw_pa->profctrl_desc.reg = AW_PID_2055A_SYSCTRL_REG;
    aw_pa->profctrl_desc.mask = AW_PID_2055A_RCV_MODE_MASK;
    aw_pa->profctrl_desc.spk_mode = AW_PID_2055A_RCV_MODE_SPEAKER_VALUE;
    ...
    aw_pa->soft_rst.reg = AW_PID_2055A_ID_REG;
    aw_pa->soft_rst.reg_value = AW882XX_SOFT_RESET_VALUE;
                                 ...
                                 struct aw_fill_info *fill_info)
{
    ...
    if (reg_data == AW_PID_2055A_VERSION_VALUE)
        return aw_pid_2055a_dev_init(aw882xx, fill_info);
    else if (reg_data == AW_PID_2055_VERSION_VALUE)
        return aw_pid_2055_dev_init(aw882xx, fill_info);
    else
...
int aw882xx_init(struct aw882xx *aw882xx, struct aw_fill_info *fill_info)
{
    ...
    {
        return aw_pid_1852_dev_init(aw882xx, fill_info);
    }
#endif

#ifdef AW_PID_2013
    if (aw882xx->chip_id == PID_2013_ID)
    {
        return aw_pid_2013_dev_init(aw882xx, fill_info);
    }
#endif

#ifdef AW_PID_2032
    if (aw882xx->chip_id == PID_2032_ID)
    {
        return aw_pid_2032_dev_init(aw882xx, fill_info);
    }
#endif
    ...
    {
        return aw_pid_2055_dev_check(aw882xx, fill_info);
    }
#endif
    ...
    {
        return aw_pid_2071_dev_init(aw882xx, fill_info);
    }
#endif
    ...
    if (aw882xx->chip_id == PID_2113_ID)
    {
        ...
        return aw_pid_2113_dev_init(aw882xx, fill_info);
    }
#endif
    aw_dev_err(aw882xx->dev_index, "unsupported chip id %x", aw882xx->chip_id);
    return -EINVAL;
}
