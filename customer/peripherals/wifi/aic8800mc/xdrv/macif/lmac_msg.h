...
struct fhcustmsg_host_ota_cfm
{
    /// Status of the operation (different from 0 if unsuccessful)
    u8_l     status;
};

struct fhcustmsg_get_fw_version_cfm
{
    int8_t status;
    char version[64];
};

#define PM_LEVEL_LIGHT_SLEEP    1
#define PM_LEVEL_DEEP_SLEEP     2
struct fhcustmsg_sleep_req
{
    /// Status of the operation (different from 0 if unsuccessful)
    u8_l     sleep_lvl;
};

struct fhcustmsg_sleep_cfm
{
    int8_t status;
};
...
{
    HTTP_REQ_STATUS_OK,
    HTTP_REQ_STATUS_FAIL,
};

struct fhcustmsg_http_req
{
    uint8_t uri[HTTP_URI_BUF_MAX];
};

struct fhcustmsg_http_resp_ind
{
    int8_t status;
    uint16_t data_len;
    uint32_t total_len;
