...
extern unsigned char* cst820_app_bin;

extern uint32_t cst820_i2c_write(uint8_t deive_addr ,uint16_t reg, uint8_t *p_data, uint16_t len);
extern uint32_t cst820_i2c_read(uint8_t deive_addr, const uint16_t reg, uint8_t *p_data, uint16_t len);
...
bool TP_HRS_WriteBytes_updata(uint8_t device_addr,uint16_t reg, uint8_t *data,uint16_t len,uint8_t lenth)
{
    ...
    ret = cst820_i2c_write(device_addr, reg, data, len);
...
{   
    ...
    ret = cst820_i2c_read(device_addr, reg, data,  len);
...
// For CSK0xx update
  ...
  *
  */	
static int cst820s_enter_bootmode(void)
{
     char retryCnt = 50;
...
static int cst820s_update(uint16_t startAddr,uint16_t len,const unsigned char *src)
{
   ...
   *
   */
 static uint32_t cst820s_read_checksum(uint16_t startAddr,uint16_t len)
 {
      union{
          uint32_t sum;
          uint8_t buf[4];
 ...
 //  TP_init();
     // test_data=cst820s_enter_bootmode();
     //hctp_i2c_init(UP_ADDR ,50);     
    if (cst820s_enter_bootmode() == 0)
    {
         LOG_I("cst820s_enter_bootmode()=%x \r\n",cst820s_enter_bootmode());
         if(sizeof(cst820_app_bin) > 10)
         {
             uint16_t startAddr = cst820_app_bin[1];
             uint16_t length = cst820_app_bin[3];
                         
             checksum = cst820_app_bin[5];
             startAddr <<= 8; startAddr |= cst820_app_bin[0];
             length <<= 8; length |= cst820_app_bin[2];
             checksum <<= 8; checksum |= cst820_app_bin[4];   
             if(cst820s_read_checksum(startAddr, length)!= checksum)
             {
                LOG_I("startAddrO=%d \r\n",startAddr);
                LOG_I("checksum=%x \r\n",checksum);
                cst820s_update(startAddr, length, &cst820_app_bin[6]);
                LOG_I("startAddrT=%d \r\n",startAddr);
                LOG_I("checksum_new=%x \r\n",checksum);
                cst820s_read_checksum(startAddr, length);        
             }
         }
				 BSP_TP_Reset(0);
				 delay_ms(20);
