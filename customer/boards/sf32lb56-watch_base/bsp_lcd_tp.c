...
#ifdef AMOLED_POWER_EN
    BSP_GPIO_Set(AMOLED_POWER_EN, 0, 1);
#endif

    BSP_LCD_Reset(0);
#if defined(AMOLED_1V8_EN) && defined(LCD_USING_TT151AMC60C)
    BSP_GPIO_Set(AMOLED_1V8_EN, 0, 0);
#endif

#if defined(AMOLED_DISPLAY_3V3_EN) && defined(PPG_POWER_EN_PIN)
    #if (AMOLED_DISPLAY_3V3_EN > 2)
    BSP_GPIO_Set(AMOLED_DISPLAY_3V3_EN, 0, 0);
    #else
    HAL_PBR_WritePin(AMOLED_DISPLAY_3V3_EN, 0);
    #endif
#endif

#ifdef AMOLED_BATTERY_EN
    BSP_GPIO_Set(AMOLED_BATTERY_EN, 0, 0);
#endif
}
...
#ifdef AMOLED_BATTERY_EN
    BSP_GPIO_Set(AMOLED_BATTERY_EN, 1, 0);
#endif

#ifdef AMOLED_DISPLAY_3V3_EN
    #if (AMOLED_DISPLAY_3V3_EN > 2)
    {
        BSP_GPIO_Set(AMOLED_DISPLAY_3V3_EN, 1, 0);
    }
    #else
    {
        HAL_PBR_WritePin(AMOLED_DISPLAY_3V3_EN, 1);
    }
    #endif
#endif

#if defined(AMOLED_1V8_EN)
    BSP_GPIO_Set(AMOLED_1V8_EN, 1, 0);
#endif

#ifdef AMOLED_POWER_EN
    BSP_GPIO_Set(AMOLED_POWER_EN, 1, 1);
#endif
}
...
void HAL_LCDC_SoftSpiInit(SOFT_SPI_PIN_Def pin, SOFT_SPI_IO_Def inout,
                          uint32_t high1low0)
{
...
{
    HAL_PIN_Set(PAD_PA00 + soft_spi_cfg[pin][0], soft_spi_cfg[pin][1],
                soft_spi_cfg[pin][2], 1);
}
