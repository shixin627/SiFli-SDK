...
{
#define sizeof_hostname 1024
#define sizeof_proto    1024
#define sizeof_path     1024
    ...
    char buf[256];
    int ret;
    char *hostname, *proto, *path;
    char portstr[10];
    s->open_timeout = 5000000;
    hostname = av_malloc(sizeof_hostname);
    proto = av_malloc(sizeof_proto);
    path = av_malloc(sizeof_path);
    RT_ASSERT(hostname && proto &&path);

    av_url_split(proto, sizeof_proto, NULL, 0, hostname, sizeof_hostname,
        &port, path, sizeof_path, uri);
    if (strcmp(proto, "tcp"))
    {
        ret = AVERROR(EINVAL);
        goto free_exit;
