...
void lv_img_set_x_frac(lv_obj_t * obj, uint16_t x_frac)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);

    lv_img_t * img = (lv_img_t *)obj;

    img->coord_x_frac = x_frac;
    lv_obj_invalidate(obj);
}

void lv_img_set_y_frac(lv_obj_t * obj, uint16_t y_frac)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);

    lv_img_t * img = (lv_img_t *)obj;

    img->coord_y_frac = y_frac;
    lv_obj_invalidate(obj);
}
    ...
    if(code == LV_EVENT_COVER_CHECK) {
        ...
        if(code == LV_EVENT_DRAW_MAIN) {
            ...
            if(img->src_type == LV_IMG_SRC_FILE || img->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                img_dsc.coord_x_frac = img->coord_x_frac;
                img_dsc.coord_y_frac = img->coord_y_frac;
