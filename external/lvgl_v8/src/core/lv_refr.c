...
void lv_obj_redraw(lv_draw_ctx_t * draw_ctx, lv_obj_t * obj)
{
    ...
    LV_DEBUG_OBJ_START_DRAW(obj, clip_area_ori);
    ...
    LV_DEBUG_OBJ_STOP_DRAW(obj, clip_area_ori);
}
...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
...
#if LV_USE_PERF_MONITOR == 0 && LV_USE_MEM_MONITOR == 0 && CONFIG_SILFI_PERF_MONITOR == 0
        /**
    ...
    extern void lv_ex_process_data(void);
    lv_ex_process_data();

#if !(defined(WIN32)||defined(_MSC_VER))
    extern bool lv_lcd_draw_error();
    if(lv_lcd_draw_error())
    {
        lv_area_t scr_area;
        scr_area.x1 = 0;
        scr_area.y1 = 0;
        ...
        _lv_inv_area(disp_refr, &scr_area);
    }
#endif /* WIN32 */

    LV_DEBUG_MARK_START(0xC1,"Update layout1");

    /*Refresh the screen's layout if required*/
    lv_obj_update_layout(disp_refr->act_scr);
    if(disp_refr->prev_scr) lv_obj_update_layout(disp_refr->prev_scr);
    LV_DEBUG_MARK_STOP(0xC1);

    LV_DEBUG_MARK_START(0xC2,"Update layout2");

    lv_obj_update_layout(disp_refr->top_layer);
    
    LV_DEBUG_MARK_STOP(0xC2);
    
    LV_DEBUG_MARK_START(0xC3,"Update layout3");
    lv_obj_update_layout(disp_refr->sys_layer);
    LV_DEBUG_MARK_STOP(0xC3);
    ...
    LV_DEBUG_MARK_START(0xC4,"Update layout3");

    refr_invalid_areas();
    LV_DEBUG_MARK_STOP(0xC4);
...
#if (0 == CONFIG_SILFI_PERF_MONITOR)
...
#endif /* (0 == CONFIG_SILFI_PERF_MONITOR) */
    }

#if  CONFIG_SILFI_PERF_MONITOR
    elaps = lv_tick_elaps(start);
...
#endif /* CONFIG_SILFI_PERF_MONITOR */
 ...
 *   STATIC FUNCTIONS
 **********************/
static int all_joined_area_intersect(lv_area_t *refresh_area, lv_area_t *joined_area)
{
    int started = 0;
    int32_t i;

    for (i = 0; i < (int)disp_refr->inv_p; i++)
    {
        if (disp_refr->inv_area_joined[i] == 0) {
            lv_area_t temp;

            if(_lv_area_intersect(&temp, &(disp_refr->inv_areas[i]), refresh_area))
            {
                if (started==0) {
                    *joined_area=temp;
                    started=1;
                    ...
                    temp2=*joined_area;
                    _lv_area_join(joined_area, &temp, &temp2);
                }
            }
        }
    }
...
#if defined(BSP_USING_RAMLESS_LCD) || defined(LCD_FB_USING_TWO_COMPRESSED)||defined(LCD_FB_USING_TWO_UNCOMPRESSED) || defined(LCD_USING_EPD_DPI)
static void lv_refr_join_area(void)
{
...
static bool get_next_refr_area(const lv_disp_t *disp, lv_area_t *area)
{
    ...
    else
    {
        ...
        if (area->y2 >= LV_VER_RES_MAX - 1) return false;
...
/**
 * Reorganize the inv areas, make every area's y1 > previous area's y2
 */
static void lv_refr_join_area(void)
{
    ...
    if(disp_refr->driver->rotated == LV_DISP_ROT_180)
    {
        ...
        refresh_area.y1 = refresh_area.y2 - (update_rows - 1);
    }
    else
    {
        refresh_area.y1 = 0;
        refresh_area.y2 = refresh_area.y1 + (update_rows - 1);
    }
    ...
    do{    
        ...
        started=all_joined_area_intersect(&refresh_area,&joined_area);

        if (started) 
        {
            if(new_inv_p < LV_INV_BUF_SIZE)
            {
                lv_area_copy(&new_inv_areas[new_inv_p++], &joined_area);
                /*rt_kprintf("new_inv_areas[%d] x1~x2=%d,%d,y1~y2=%d,%d\r\n", 
                                    new_inv_p - 1,
                                    joined_area.x1,   joined_area.x2,
                                    joined_area.y1,   joined_area.y2);
                */
            }
            else
            {
                ...
                lv_area_copy(&new_inv_areas[0], &scr_area);
                new_inv_p = 1;
                break;
            }
        }

     }while(get_next_refr_area(disp_refr, &refresh_area));

    /*Overwrite previous inv_area*/
    lv_memset_00(disp_refr->inv_area_joined, sizeof(disp_refr->inv_area_joined));
...
#endif /* LV_FRAME_BUF_SCHEME_5 */


#if defined(LV_FB_ONE_SCREEN_SIZE) && defined(LCD_FB_USING_NONE)
extern void wait_writeable_area(const lv_area_t *a_in, lv_area_t *a_out);
...
static void refr_invalid_areas(void)
{
    ...
    //rt_kprintf("====lv_refr_areas=====\r\n");
    lv_area_t scan_area = {0, 0, LV_HOR_RES_MAX - 1, LV_VER_RES_MAX - 1};
    lv_area_t refresh_area, writeable_area;
    int started;

    started = all_joined_area_intersect(&scan_area, &refresh_area);
    ...
    do
    {
        ...
        {
            PRINT_AREA("\r\nscan_area", &scan_area);
            PRINT_AREA("unaligned refresh_area", &refresh_area);
            if (disp_refr->driver->rounder_cb)
            {
                disp_refr->driver->rounder_cb(disp_refr->driver, &refresh_area);
            }
            PRINT_AREA("aligned refresh_area", &refresh_area);

            wait_writeable_area(&refresh_area, &writeable_area);
            PRINT_AREA("real refresh_area", &writeable_area);

            RT_ASSERT((writeable_area.x1 <= writeable_area.x2) && (writeable_area.y1 <= writeable_area.y2));

            //Remove written area from scan_area
            scan_area.y1 = writeable_area.y2 + 1;

            if (scan_area.y1 < LV_VER_RES_MAX)
            {
                //Calculate next refresh area
                started = all_joined_area_intersect(&scan_area, &refresh_area);
            }
            else
            {
            ...
            disp_refr->driver->draw_buf->last_area = (0 == started) ? 1 : 0;
            disp_refr->driver->draw_buf->last_part = 1;
            refr_area(&writeable_area);

            px_num += lv_area_get_size(&writeable_area);
        ...
        {
            break; //All invalid areas were refreshed.
        }
...
#else /* LV_FB_ONE_SCREEN_SIZE && LCD_FB_USING_NONE*/
...
static void call_flush_cb(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    LV_DEBUG_VDB_START_FLUSH(color_p,&offset_area);
    drv->flush_cb(drv, &offset_area, color_p);
    LV_DEBUG_VDB_STOP_FLUSH(color_p,&offset_area);
}
