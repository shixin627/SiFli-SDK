...
void FTC_Manager_Cache_Free( FTC_Manager  manager, unsigned int max_weight)
	{
	  FTC_Node	 node, first;
	  ...
	  /*rt_kprintf( "Cache_Free, weight = %ld, max = %ld, nodes = %d, reserved weight = %d\n",
	  			manager->cur_weight, manager->max_weight,
	  			manager->num_nodes, max_weight);*/
	  			
	  if ( manager->cur_weight < max_weight || !first )
		return;
	  ...
	  } while ( node && manager->cur_weight > max_weight );
	}
