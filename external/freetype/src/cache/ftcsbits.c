...
static ft_bitmap_to_bpp_func bitmap_to_bpp_func = NULL;;
static int g_bpp = 0;
...
void ft_bitmap_to_bpp_register(ft_bitmap_to_bpp_func func, int bpp)
{
	bitmap_to_bpp_func = func;
	g_bpp = bpp;
}
/*call back end*/
...
#define CHECK_SHRT( d )  ( temp = (FT_Short)d, (FT_Int)temp == (FT_Int) d )
	  ...
	  {
		if (bitmap_to_bpp_func)
		{
			error = bitmap_to_bpp_func(sbit, bitmap, manager->memory);
		}
		else 
		{
