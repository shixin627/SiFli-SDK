                                                         ...
                                                         q7_t * bufferB)
{
    ...
    config.wt = wt;
    config.bias = bias;
    config.output = Im_out;
    config.in_dim_x = dim_im_in_x;
    config.in_dim_y = dim_im_in_y;
    config.in_ch_num = ch_im_in;
    config.kernel_dim_x = dim_kernel_x;
    config.kernel_dim_y = dim_kernel_y;
    config.padding_x = padding_x;
    config.padding_y = padding_y;
    config.stride_x = stride_x;
    config.stride_y = stride_y;
    config.out_dim_x = dim_im_out_x;
    config.out_dim_y = dim_im_out_y;
    config.out_ch_num = ch_im_out;
    config.bias_shift = bias_shift;
    config.out_shift = out_shift;
    config.mode = HAL_NNACC_MODE_DEPTHWISE_CONV2D;
    err = nn_acc_start(&config);
