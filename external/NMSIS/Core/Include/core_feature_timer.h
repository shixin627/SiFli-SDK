...
__STATIC_FORCEINLINE void SysTimer_AutoStart(void)
{
    SysTimer->MTIMECTL |= SysTimer_MTIMECTL_CMPCLREN_Msk;
}

__STATIC_FORCEINLINE void SysTimer_Clear_AutoStart(void)
{
    SysTimer->MTIMECTL &= ~(SysTimer_MTIMECTL_TIMESTOP_Msk);
}


__STATIC_FORCEINLINE void SysTimer_Set_MTIMECTRL(uint8_t is_stop, uint8_t is_clear, uint8_t src)
{
    
    SysTimer->MTIMECTL = ((is_stop << SysTimer_MTIMECTL_TIMESTOP_Pos) & SysTimer_MTIMECTL_TIMESTOP_Msk) | 
                         ((is_clear << SysTimer_MTIMECTL_CMPCLREN_Pos) & SysTimer_MTIMECTL_CMPCLREN_Msk) |
                         ((src << SysTimer_MTIMECTL_CLKSRC_Pos) & SysTimer_MTIMECTL_CLKSRC_Msk);
}
...
{
    SysTimer->MTIMECTL = (mctl & SysTimer_MTIMECTL_CMPCLREN_Pos);
}
