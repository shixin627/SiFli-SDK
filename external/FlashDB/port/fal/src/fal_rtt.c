...
static rt_err_t mtd_nor_dev_control(struct rt_mtd_nor_device *device, int cmd, void *args)
{
    ...
    if (RT_DEVICE_CTRL_GET_PHY_ADDR == cmd)
    {
        ...
        flash_dev = fal_flash_device_find(part->fal_part->flash_name);
        if (flash_dev)
        {
            /* physical_address =  flash_base_address + partition_offset + offset_in_partition */
            addr_mapping->physical_addr = flash_dev->addr + part->fal_part->offset + addr_mapping->logical_addr;
            ret = RT_EOK;
        }
    }
...
#include "drv_flash.h"
static rt_uint32_t mtd_nand_readid(struct rt_mtd_nand_device *device)
{
    struct fal_flash_dev *fal_dev;
    
    fal_dev = (struct fal_flash_dev *)device->parent.user_data;

    return rt_flash_read_id(fal_dev->addr);
}

static rt_err_t mtd_nand_readpage(struct rt_mtd_nand_device *device,
                              rt_off_t page,
                              ...
                              rt_uint8_t *spare, rt_uint32_t spare_len)
{
    ...
    res = rt_nand_read_page(fal_dev->addr + page * device->page_size, data, data_len, spare, spare_len);
...
static rt_err_t mtd_nand_readpage_with_offset(struct rt_mtd_nand_device *device,
        ...
        rt_uint8_t *spare, rt_uint32_t spare_len)
{
    ...
    res = rt_nand_read_page(fal_dev->addr + page * device->page_size + offset, data, data_len, spare, spare_len);
    if (res <= 0)
    {
        log_i("rt_nand_read_page RES %d\n", res);
        return RT_MTD_ENOMEM;
                               ...
                               const rt_uint8_t *spare, rt_uint32_t spare_len)
{
    ...
    res = rt_nand_write_page(fal_dev->addr + page * device->page_size, data, data_len, spare, spare_len);
...
static rt_err_t mtd_nand_movepage(struct rt_mtd_nand_device *device, rt_off_t src_page, rt_off_t dst_page)
{
    struct fal_flash_dev *fal_dev;
    int res;
...
static rt_err_t mtd_nand_eraseblk(struct rt_mtd_nand_device *device, rt_uint32_t block)
{
    struct fal_flash_dev *fal_dev;
    int res;
    ...
    res = rt_nand_erase_block((block << 17) + fal_dev->addr);
    //res = rt_flash_erase((block << 17) + fal_dev->addr, fal_dev->blk_size);
    if (res)
    {
        return RT_MTD_EIO;
    }
...
static rt_err_t mtd_nand_checkblk(struct rt_mtd_nand_device *device, rt_uint32_t block)
{
    return 0;
}

static rt_err_t mtd_nand_mark_badblk(struct rt_mtd_nand_device *device, rt_uint32_t block)
{
    return 0;
}
...
static const struct rt_mtd_nand_driver_ops spi_nand_ops =
{
    ...
    .check_block = mtd_nand_checkblk,
    .mark_badblock = mtd_nand_mark_badblk,
};
#endif


#if defined(RT_USING_MTD_NOR) || defined(RT_USING_MTD_NAND)
/**
...
struct rt_device *fal_mtd_device_create(const char *parition_name)
{
...
#ifdef RT_USING_MTD_NAND    
    struct fal_mtd_nand_device *mtd_nand_dev;    
#endif
    ...
    if (!fal_flash->nand_flag)
    {
        ...
        if (mtd_nor_dev)
        {
            ...
            if (FAL_FS_PART_FLAG & fal_part->reserved) //is filesystem
            {
                mtd_nor_dev->parent.block_size = QSPI_NOR_BLK64_SIZE;
                mtd_nor_dev->parent.sector_size = 2048;
            }
#else
            if (false)
            {
            }
#endif /* RT_DFS_ELM_DHARA_ENABLED */
            else
            {
                mtd_nor_dev->parent.block_size = fal_flash->blk_size;
                mtd_nor_dev->parent.sector_size = fal_flash->sector_size;
            }

            mtd_nor_dev->parent.block_end = fal_part->len / mtd_nor_dev->parent.block_size;
    ...
    {
        #if 1//!defined(SOC_SF32LB58X)
        mtd_nand_dev = (struct fal_mtd_nand_device*) rt_malloc(sizeof(struct fal_mtd_nand_device));
        if (mtd_nand_dev)
        {
            mtd_nand_dev->fal_part = fal_part;

            mtd_nand_dev->parent.page_size = fal_flash->sector_size;
            mtd_nand_dev->parent.oob_size = 64;
            mtd_nand_dev->parent.oob_free = 24;
            mtd_nand_dev->parent.plane_num = 2;
            mtd_nand_dev->parent.pages_per_block = fal_flash->blk_size / fal_flash->sector_size;
            mtd_nand_dev->parent.block_total = fal_part->len / fal_flash->blk_size;
            mtd_nand_dev->parent.block_start = fal_part->offset / fal_flash->blk_size;
            mtd_nand_dev->parent.block_end = mtd_nand_dev->parent.block_start + mtd_nand_dev->parent.block_total - 1;
            mtd_nand_dev->parent.cache_buf = NULL;
            mtd_nand_dev->parent.blk_cnum = 0;
            mtd_nand_dev->parent.ops = &spi_nand_ops;
            mtd_nand_dev->parent.parent.user_data = (void *)fal_flash;

            log_i("The FAL MTD NAND device (%s)  blk:%d sec:%d created successfully", fal_part->name,fal_flash->blk_size,fal_flash->sector_size);
			rt_kprintf("Register0 %s to mtd device with base addr 0x%08x\n", fal_part->name, mtd_nand_dev->parent.block_start * fal_flash->blk_size + fal_flash->addr);
		    rt_kprintf("Register1%s to mtd device with base addr 0x%08x\n", fal_part->name, fal_part->offset + fal_flash->addr);
			rt_kprintf("Register2 %s to mtd device with base addr 0x%08x\n", fal_part->name, fal_flash->addr+fal_part->offset);
            rt_mtd_nand_register_device(fal_part->name, &mtd_nand_dev->parent);
        ...
        #else
            register_nand_device(fal_flash->addr, fal_part->offset, fal_part->len, (char*)fal_part->name);
        #endif
    }
#endif    
    ...
    {
#if defined(RT_USING_MTD_NOR)
        dev = RT_DEVICE(&mtd_nor_dev->parent);
#endif
    }
#ifdef RT_USING_MTD_NAND    
    else
    {
        dev = RT_DEVICE(&mtd_nand_dev->parent);
    }
#endif
