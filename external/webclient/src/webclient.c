...
int webclient_send_usermethod(struct webclient_session *session, const char *uri, const void *body, size_t body_len)
{
    int rc = WEBCLIENT_OK;
    int resp_status = 0;
    ...
    if (rc != WEBCLIENT_OK)
    {
        LOG_E("webclient_connect err2");
        return rc;
    }

    if (body && (body_len > 0))
    {
        webclient_write(session, body, body_len);
    }
