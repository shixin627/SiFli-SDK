...
uint16_t g_aecm_real_shift = 13;
uint16_t g_aecm_imag_shift = 13;
                            ...
                            int16_t *output)
{
    ...
    uint16_t *xfa = malloc(PART_LEN1 * 2);
    uint16_t *dfaNoisy = malloc(PART_LEN1 * 2);
    uint16_t *dfaClean = malloc(PART_LEN1 * 2);
    // 32 byte aligned buffers (with +8 or +16).
    // TODO(kma): define fft with ComplexInt16.
    int16_t *fft_buf = malloc((PART_LEN4 + 2 + 16) * 2); // +2 to make a loop safe.
    int32_t *echoEst32_buf = malloc((PART_LEN1 + 8) * 4);
    int32_t *dfw_buf = malloc((PART_LEN2 + 8) * 4);
    int32_t *efw_buf = malloc((PART_LEN2 + 8) * 4);
    int16_t *hnl = malloc(PART_LEN1 * 2);
    assert(xfa && dfaNoisy && dfaClean && fft_buf && echoEst32_buf && dfw_buf && efw_buf && hnl);
#else
    ...
    // Whistle detection start
    uint32_t subEnergy[16];
    uint16_t subPeak[16];
    uint32_t subLow = 0;
    uint32_t subHigh = 0;
    ...
    if (BT_DEV_CLS_AUDIO_BOX == bt_connect_get_peer_type())
    {
        for (i = 0; i < 64; i++)
        {
            nearabs = (nearendNoisy[i] > 0) ? nearendNoisy[i] : -nearendNoisy[i];
            nearPeak = (nearabs > nearPeak) ? nearabs : nearPeak;
        }
        for (i = 0; i < 16; i++)
        {
            subEnergy[i] = 0;
        ...
        for (i = 0; i < 14; i++)
        {
            for (j = 0; j < 8; j++)
            {
                currentPower = ptrDfaClean[i * 4 + j+2];
                subEnergy[i] += currentPower;
                subPeak[i] = (currentPower > subPeak[i]) ? currentPower : subPeak[i];
            }
        }
        for (i = 0; i < 32; i++)
        {
            subLow += ptrDfaClean[i];
            subHigh += ptrDfaClean[i + 32];
        }
        whistleDet = 0;
        for (i = 4; i < 14; i++)
        {
            if (subEnergy[i] < subEnergy[0] * 3)   //when low frequency dominates, whistle do not exist
                continue;
            if (subPeak[i] > 5000)
            {
                whistleDet = 1;
            ...
            if (aecm->whistleCount > 0)
                continue;
            if ((subEnergy[i] > subEnergy[0] * 3))
                whistleDet = 1;
        }
        if ((aecm->whistleCount == 0) && (subHigh > subLow))
            whistleDet = 1;
        for (i = 1; i < 4; i++)
        {
            if ((subEnergy[i] > 4000) && (subEnergy[9] < 3000) && (nearPeak > 2000) && (subHigh*2 < subLow))
                aecm->whistleCount = 200;
        }
        if (aecm->whistleCount > 0)
            aecm->whistleCount--;
    }
    // Whistle detection end
#endif
