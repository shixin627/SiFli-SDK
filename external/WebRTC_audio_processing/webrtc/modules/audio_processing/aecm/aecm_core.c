                            ...
                            int16_t *out)
{
    ...
    int16_t *outBlock_buf = malloc((PART_LEN + 8) * 2); // Align buffer to 8-byte boundary.
    int16_t *farFrame = malloc(FRAME_LEN * 2);
    int16_t *far_block = malloc(PART_LEN * 2);;
    int16_t *near_noisy_block = malloc(PART_LEN * 2);
    int16_t *near_clean_block = malloc(PART_LEN * 2);
    assert(outBlock_buf && farFrame && far_block && near_noisy_block && near_clean_block);
#else
...
err:
#ifdef AUDIO_MEM_ALLOC
    free(near_clean_block);
    free(near_noisy_block);
    free(far_block);
    free(farFrame);
    free(outBlock_buf);
#endif
