...
#define CLR_USB_ON      lv_color_hex(0x00E676)  /* USB connected dot+text  */
#define CLR_USB_OFF     lv_color_hex(0x333333)  /* USB disconnected dot    */
 ...
 *============================================================================*/
/* Top bar */
static lv_obj_t *s_usb_dot   = NULL;   /* small circle indicator  */
static lv_obj_t *s_usb_label = NULL;   /* "USB" text              */
...
static rt_err_t ui_create(float scale)
{
    ...
    static lv_style_t st_small;     /* USB label, source     */
    ...
    /* ── Top bar: USB dot + label ── */
    /* Dot — a tiny 8×8 circle */
    s_usb_dot = lv_obj_create(scr);
    lv_obj_remove_flag(s_usb_dot, LV_OBJ_FLAG_SCROLLABLE);
    {
        int dot_sz = (int)(8 * scale);
        if (dot_sz < 6) dot_sz = 6;
        lv_obj_set_size(s_usb_dot, dot_sz, dot_sz);
    }
    lv_obj_set_style_radius(s_usb_dot, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(s_usb_dot, CLR_USB_OFF, 0);
    lv_obj_set_style_bg_opa(s_usb_dot, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(s_usb_dot, 0, 0);
    lv_obj_set_align(s_usb_dot, LV_ALIGN_TOP_MID);
    lv_obj_set_x(s_usb_dot, (int)(-20 * scale));
    lv_obj_set_y(s_usb_dot, (int)(20 * scale));

    /* USB label */
    s_usb_label = lv_label_create(scr);
    lv_label_set_text(s_usb_label, "USB");
    lv_obj_add_style(s_usb_label, &st_small, 0);
    lv_obj_set_style_text_color(s_usb_label, CLR_TEXT_DIM, 0);
    lv_obj_set_style_text_letter_space(s_usb_label, (int)(2 * scale), 0);
    lv_obj_set_align(s_usb_label, LV_ALIGN_TOP_MID);
    lv_obj_set_x(s_usb_label, (int)(8 * scale));
    lv_obj_set_y(s_usb_label, (int)(16 * scale));
...
static void handle_usb(const char *data)
{
    if (!data) return;

    bool on = (strcmp(data, UI_MSG_DATA_USB_CONNECTED) == 0);

    if (s_usb_dot)
    {
        lv_obj_set_style_bg_color(s_usb_dot, on ? CLR_USB_ON : CLR_USB_OFF, 0);
        if (on)
        {
            lv_obj_set_style_shadow_color(s_usb_dot, CLR_USB_ON, 0);
            lv_obj_set_style_shadow_width(s_usb_dot, (int)(8 * g_scale), 0);
            lv_obj_set_style_shadow_spread(s_usb_dot, (int)(2 * g_scale), 0);
            lv_obj_set_style_shadow_opa(s_usb_dot, LV_OPA_50, 0);
        }
        else
        {
            lv_obj_set_style_shadow_width(s_usb_dot, 0, 0);
        }
    }

    if (s_usb_label)
    {
        lv_obj_set_style_text_color(s_usb_label, on ? CLR_USB_ON : CLR_TEXT_DIM, 0);
    }
...
static void handle_source(const char *data)
{
    ...
    else if (strcmp(data, UI_MSG_DATA_SOURCE_USB) == 0)
    {
        lv_label_set_text(s_source, "[CDC]");
        lv_obj_set_style_text_color(s_source, CLR_USB_ON, 0);
    }
...
void dfu_ui_task(void *args)
{
    ...
    while (1)
    {
               ...
               == RT_EOK)
        {
            ...
            /* BLE / NET — silently ignore (no icons in new UI) */
