...
* 再编译界面中输入`sdk.py menuconfig --board=sf32lb52-lcd_52d`进入菜单，在`(Top) → On-chip Peripheral RTOS Drivers`下进行如下配置
...
在进行连接虚拟串口USB即可
 ...
 1.  修改配置sdk.py menuconfig --board=sf32lb52-lcd_n16r8中重新修改 将"usb Insertion detection PIN"中的参数修改为想要的检测引脚
