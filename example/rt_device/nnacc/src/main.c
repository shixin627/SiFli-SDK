...
// Uncomment to use asynchronous mode
#define async_nnacc
...
#define NN_ROUND( out_shift ) ((( uint32_t )1 << out_shift) >> 1)
    ...
    {
        //rt_kprintf( "ref row cal done\n" );
        for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
        {
            for (i_ch_out = 0; i_ch_out < ch_im_out; i_ch_out++)
            {
                //rt_kprintf( "ref cal begin\n" );
                // for each output
                ...
                if (conv_out_sat != data)
                {
                    if (max_log > 0)
                    {
                        rt_kprintf("out_x=%d out_y=%d out_ch=%d, rslt=%x, rslt_sat=%x nn_rslt=%x", i_out_x, i_out_y, i_ch_out, conv_out, conv_out_sat, data);
                        max_log--;
                    }
...
static  int  init_buffers(void)
{
    ...
    else
    {
        ...
        rt_kprintf("init_buffers failed.\n");
        return HAL_ERROR;
    }
}
static volatile uint8_t async_done = 0;
void nnacc_async_complete_callback(void)
{
    rt_kprintf("Async operation complete.\n");
    async_done = 1;
}
...
int main(void)
{
    ...
    if (HAL_OK == ret)
    {
        rt_kprintf("Buffers initialized successfully.\n");
    }
    else
    {
        rt_kprintf("Failed to initialize buffers.\n");
        return 1;
    }

    rt_kprintf("fill_with_random_data. %x %x %x  %x, sp=%x\n", config.wt, config.bias, config.input, config.output, &ret);
    ...
    rt_kprintf("HAL_NNACC_Start.\n");


    // Initiate reasoning.
#if defined(sync_nnacc)
    ret = nn_acc_start(&config);
#elif defined(async_nnacc)
    async_done = 0;
    ret = nn_acc_start_IT(&config, nnacc_async_complete_callback);
    ...
    if (ret == HAL_OK)
    {
        ...
        {
            rt_kprintf("Test passed: NNACC result is correct.\n");
        }
        else
        {
            rt_kprintf("Test failed: NNACC result mismatch.\n");
        }
    }
    else
    {
