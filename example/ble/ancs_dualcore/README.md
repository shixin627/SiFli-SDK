...
1. 例程开机会开启广播，广播名字以SIFLI_APP-xx-xx-xx-xx-xx-xx, 其中xx代表本设备的蓝牙地址。可以通过finsh命令"nvds get_mac"获取。需要注意的是，该工程默认启用睡眠模式，一旦运行会很快进入睡眠模式。进入睡眠模式后，无法连接Jlink，Uart也无法输入，蓝牙可以正常工作。如果有输入finsh命令的需求，可以把PA80接地来强制不进入睡眠模式。(557是PA77)
