...
#ifdef BSP_BLE_DISS
// Callback function to get device information
...
#ifdef SF32LB55X
#ifdef BSP_BLE_DISS
            sibles_ble_diss_init(dis_app_callback); //The SF32LB55X can only enable DISs through the sibles_ble_diss_init() interface.
#endif
#endif
...
/*
The SF32LB55X can only enable DISS through the sibles_ble_diss_init() interface.
Devices other than SF32LB55X can enable DISS by using either sibles_ble_diss_init() or ble_app_dis_enable().
*/
...
int cmd_diss(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        ...
        else if (strcmp(argv[1], "manu") == 0)
        {
            ...
            // This is the second way to set device information (the first way is using callback function dis_app_callback)
