...
源码路径：example/bt/music_sink_with_hci_over_uart/music_sink
...
2. 本例程以example/bt/music_sink工程作为基础，是作为bt/ble host的部分。


## 例程的使用
<!-- 说明如何使用例程，比如连接哪些硬件管脚观察波形，编译和烧写可以引用相关文档。
对于rt_device的例程，还需要把本例程用到的配置开关列出来，比如PWM例程用到了PWM1，需要在onchip菜单里使能PWM1 -->
1. 一块开发板烧录本工程，另一块开发板烧录music_sink_with_hci_over_uart/HCI_over_uart工程
...
8. HCI切换到uart
    - 路径：Sifli middleware → Bluetooth → Bluetooth Hardware Config → Select statck data transfer port
    - 开启：Use UART as transfer port
        - 宏开关：`CONFIG_UART_PORT0`
        - 作用：HCI数据传输端口使用uart
    - 赋值：Choose uart port
        - 宏值：`CONFIG_UART_PORT0_PORT`
        - 作用：选择uart端口
9. HCI切换到uart
    - 路径：Sifli middleware → Bluetooth → Bluetooth Hardware Config → Select statck data transfer port
    - 开启：Use UART as transfer port
    ...
    - 赋值：Choose uart port
        ...
        - 作用：选择uart端口
10. 打开uart2
    - 路径：On-chip peripheral RTOS Drivers → Enable UART
    - 开启：Enable UART2
        - 宏开关：`CONFIG_BSP_USING_UART2`
        - 作用：使能UART2端口
    - 开启：Enable UART2 RX DMA
    - 宏开关：`CONFIG_BSP_UART2_RX_USING_DMA`
    - 作用：使能UART2 RX DMA
