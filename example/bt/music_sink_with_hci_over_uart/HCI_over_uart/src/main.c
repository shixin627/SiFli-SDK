...
#define BLE_HCI_FORWARD_PORT "uart2"
...
static void ble_wvt_uart_init(void)
{
    ...
    rt_kprintf("HCI forward find %s dev=%p\r\n", BLE_HCI_FORWARD_PORT, env->uart_port);
    if (env->uart_port)
    {
        ...
        rt_kprintf("HCI forward open %s result=%d oflag=0x%x dev_flag=0x%x\r\n",
                   BLE_HCI_FORWARD_PORT, result, oflag, env->uart_port->flag);
        RT_ASSERT(RT_EOK == result);

        rt_device_set_rx_indicate(env->uart_port, ble_wvt_uart_rx_ind);
        rt_kprintf("HCI forward rx_ind installed on %s\r\n", BLE_HCI_FORWARD_PORT);
    }
