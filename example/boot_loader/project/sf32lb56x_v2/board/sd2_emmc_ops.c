...
static uint32_t sdemmc_cache2[128];
static uint8_t  wire_mode2 = 0;  // 0 for 1-wire mode
...
int sdio2_emmc_init()
{
    ...
    //debug_print("SDMMC2 sd detect start!\n");

    //initialize sdmmc host
    sd2_init();
    hwp_sdmmc2->CLKCR = 119 << SD_CLKCR_DIV_Pos; //48M/120=400k
    hwp_sdmmc2->CLKCR |= SD_CLKCR_VOID_FIFO_ERROR;
    hwp_sdmmc2->IER = 0; //mask sdmmc interrupt
    hwp_sdmmc2->TOR = 0x02000000; //mask sdmmc interrupt
    ...
    //initialize sd card
    cmd_result = sd2_send_cmd(0, 0); //CMD0

    //set sd_req and wait for sd_busy before access sd in normal mode
    hwp_sdmmc2->CASR = SD_CASR_SD_REQ;
    while ((hwp_sdmmc2->CASR & SD_CASR_SD_BUSY) == 0);
    //debug_print("CMD 0 done\n");

    hwp_sdmmc2->CDR |= SD_CDR_CMD_OD;   // SET TO Open Drain mode
    ...
    {
        cmd_arg = 0x40000080;
        cmd_result = sd2_send_cmd(1, cmd_arg); //CMD1

        sd2_get_rsp(&rsp_idx, &rsp_arg[0], &rsp_arg[1], &rsp_arg[2], &rsp_arg[3]);
    ...
    HAL_Delay_us(20);
    cmd_arg = 0x0;
    cmd_result = sd2_send_cmd(2, cmd_arg); //CMD2
    ...
    sd2_get_rsp(&rsp_idx, &cid[3], &cid[2], &cid[1], &cid[0]);
    ...
    rca = 1;
    cmd_arg = 0x10000;
    cmd_result = sd2_send_cmd(3, cmd_arg); //CMD3
    if (cmd_result == SD_TIMEOUT)
    {
    ...
    hwp_sdmmc2->CDR &= ~SD_CDR_CMD_OD;  // recover to push pull mode

    HAL_Delay_us(20);
    cmd_arg = rca << 16;
    //cmd_arg = 0x10000;
    cmd_result = sd2_send_cmd(9, cmd_arg); //CMD9
    if (cmd_result == SD_TIMEOUT)
    {
    ...
    hwp_sdmmc2->CLKCR = 7 << SD_CLKCR_DIV_Pos; //48M/8=6M
    hwp_sdmmc2->CLKCR |= SD_CLKCR_VOID_FIFO_ERROR;
    hwp_sdmmc2->TOR = 0x02000000; // set timeout
    hwp_sdmmc2->CDR = SD_CDR_ITIMING_SEL | (0 << SD_CDR_ITIMING_Pos);
    ...
    cmd_result = sd2_send_cmd(7, cmd_arg);
    if (cmd_result == SD_TIMEOUT)
    {
    ...
    //CMD8 EXT_CSD
    hwp_sdmmc2->SR = 0xffffffff; //clear sdmmc interrupts
    sd2_read(0, 1); //1 wire mode,1 blocks
    cmd_arg = 0;
    cmd_result = sd2_send_cmd(8, cmd_arg);
    if (cmd_result == SD_TIMEOUT)
    {
    ...
    hwp_sdmmc2->IER = SD_IER_DATA_DONE_MASK;
    cmd_result = sd2_wait_read();  //wait sdmmc interrupt
    ...
    for (i = 0; i < 512 / 4; i++)
    {
        sdemmc_cache2[i] = hwp_sdmmc2->FIFO;
    }
    // parse CSD
    for (i = 0; i < 512 / 4; i++)
    {
        //debug_print(" 0x");
        //debug_print((char *)htoa(hex, sdemmc_cache2[i]));
        if ((i & 7) == 7)
        {
    ...
    // get cap
    uint32_t capacity = sdemmc_cache2[212 / 4];
    ...
    cmd_arg = cmd_arg | 1;
    cmd_result = sd2_send_cmd(6, cmd_arg); //
    if (cmd_result == SD_TIMEOUT)
    {
    ...
    sd2_read(wire_mode2, 1); //4 wire mode,1 blocks
#else
    sd2_read(0, 1); //1 wire mode,1 blocks
#endif
    ...
    cmd_result = sd2_send_cmd(17, cmd_arg);
    if (cmd_result == SD_TIMEOUT)
    {
    ...
    //debug_print("SDMMC 2 initial finish\n");
    return 0;
}
...
int emmc2_read_data(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    sd2_read(wire_mode2, 1); //4 wire mode,8 blocks
    ...
    sd2_get_rsp(&rsp_idx, &rsp_arg1, &rsp_arg2, &rsp_arg3, &rsp_arg4);
    if (rsp_idx != 17)
    {
        //debug_print("CMD17 RSP ERR!\n");
        test_result = TEST_FAIL;
...
int sd_write_data(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    hwp_sdmmc2->SR = 0xffffffff; //clear sdmmc interrupts
    cmd_arg = addr >> 9; //start data address
    cmd_result = sd2_send_cmd(24, cmd_arg);
    if (cmd_result == SD_TIMEOUT)
    {
    ...
    //start dma for write
    sd2_write(wire_mode2, 1); //4 wire mode,1 block
    hwp_sdmmc2->SR = 0xffffffff; //clear sdmmc interrupts
    hwp_sdmmc2->IER = SD_IER_DATA_DONE_MASK;
    ...
    cmd_result = sd2_wait_write();  //wait sdmmc interrupt
    if (hwp_sdmmc2->SR & SD_SR_DATA_TIMEOUT)
    {
