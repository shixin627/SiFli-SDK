...
#define DBG_TAG "activity"
#define DBG_LVL DBG_LOG
#include <rtdbg.h>

#define ENABLE_ACTIVITY_INSIGHTS 0

#define ACTIVITY_LOG_DEBUG LOG_D
// extern void ACTIVITY_LOG_DEBUG(const char *format, ...);
...
#define ACTIVITY_RAW_SAMPLE_VALUE_ENCODE(x) ((((x) + 4) >> ACTIVITY_RAW_SAMPLE_SHIFT) & ACTIVITY_RAW_SAMPLE_VALUE_MASK)
...
#define ACTIVITY_RAW_SAMPLE_GET_X(s) \
  ACTIVITY_RAW_SAMPLE_SIGN_EXTEND((((uint32_t)s >> (2 * ACTIVITY_RAW_SAMPLE_VALUE_BITS)) & ACTIVITY_RAW_SAMPLE_VALUE_MASK) << ACTIVITY_RAW_SAMPLE_SHIFT)
#define ACTIVITY_RAW_SAMPLE_GET_Y(s) \
  ACTIVITY_RAW_SAMPLE_SIGN_EXTEND(((s >> ACTIVITY_RAW_SAMPLE_VALUE_BITS) & ACTIVITY_RAW_SAMPLE_VALUE_MASK) << ACTIVITY_RAW_SAMPLE_SHIFT)
...
#define ACTIVITY_RAW_SAMPLE_ENCODE(run_size, x, y, z) \
  ((run_size) << (3 * ACTIVITY_RAW_SAMPLE_VALUE_BITS)) | (ACTIVITY_RAW_SAMPLE_VALUE_ENCODE(x) << (2 * ACTIVITY_RAW_SAMPLE_VALUE_BITS)) | (ACTIVITY_RAW_SAMPLE_VALUE_ENCODE(y) << ACTIVITY_RAW_SAMPLE_VALUE_BITS) | ACTIVITY_RAW_SAMPLE_VALUE_ENCODE(z)
