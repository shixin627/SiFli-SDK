                                                                 ...
                                                                 bool remove_ongoing)
{
  ...
  for (uint32_t i = 0; i < *session_entries; i++)
  {
    ...
    if ((end_time >= end_utc) && (end_time <= utc_sec) && (!remove_ongoing || !sessions[i].ongoing))
    {
      // Keep it
      continue;
    }
                                          ...
                                          time_t start_utc, time_t end_utc)
{
  ...
  {
    if ((session->type != ActivitySessionType_RestfulSleep) && (session->type != ActivitySessionType_RestfulNap))
    {
      continue;
    }
    if ((session->start_utc >= start_utc) && ((time_t)(session->start_utc + (session->length_min * SECONDS_PER_MINUTE)) <= end_utc))
    {
                                    ...
                                    ActivitySleepStats *stats)
{
  ...
  *stats = (ActivitySleepStats){};
  ...
  for (uint32_t i = 0; i < state->activity_sessions_count; i++, session++)
  {
    ...
    if ((session->type == ActivitySessionType_Sleep) || (session->type == ActivitySessionType_Nap))
    {
      rv = true;
    ...
    else if ((session->type == ActivitySessionType_RestfulSleep) || (session->type == ActivitySessionType_RestfulNap))
    {
      if (session_exit_utc <= max_end_utc)
      {
                                     ...
                                     time_t last_processed_utc)
{
  ...
  mutex_lock_recursive(state->mutex);
  {
    ...
    if ((sleep_data->total_minutes != prev_sleep_data.total_minutes) || (sleep_data->restful_minutes != prev_sleep_data.restful_minutes))
    {
      ...
      // };
      // event_put(&e);
      watch_sys_sync.notify_sleep_state(sleep_data->total_minutes * SECONDS_PER_MINUTE, sleep_data->restful_minutes * SECONDS_PER_MINUTE);
    }

    if (sleep_data->cur_state != prev_sleep_data.cur_state)
    {
      // Debug logging
      ACTIVITY_LOG_DEBUG("total_min: %d, deep_min: %d, state: %d, state_min: %d",
...
void NOINLINE activity_sessions_prv_minute_handler(time_t utc_sec)
{
  ...
  int last_sleep_utc_of_day = time_util_get_midnight_of(utc_sec) + ACTIVITY_LAST_SLEEP_MINUTE_OF_DAY * SECONDS_PER_MINUTE;
