...
#define GESTURE_TAP_TIME_STEP 35
...
#define DEVICE_REGISTRY_VERSION 2
    ...
    typedef struct
    {
        ...
        /* 1: BYPASS the wear-detection (contact) algorithm (forced worn).
         * 0 = run it, and 0 is the DEFAULT (founder 2026-09-10: detection on by
         * default again). Same bit position as the 2026-09-03 wear_detect_on,
         * with the meaning inverted on purpose, so every blob saved while the
         * default was OFF now reads as ON without a migration. A watch whose
         * user had explicitly switched detection ON under the old meaning
         * reads as bypass once and needs one toggle. */
        uint64_t wear_detect_off : 1;
