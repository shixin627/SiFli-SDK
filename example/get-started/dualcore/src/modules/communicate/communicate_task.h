...
bool commu_send_device_drag(int permille, const char *to_device_id, int to_tab); /* -> KEY_DEVICE_DRAG (0x26) */
bool commu_send_phone_tab_pick(int index); /* -> KEY_PHONE_TAB_PICK (0x2d) */
...
bool commu_send_conv_avatar_req(const char *av);                             /* -> KEY_CONV_AVATAR_REQ (0x25) 要一張還沒有的 Bot 頭像(內容雜湊當鍵) */
...
bool commu_send_skaiapp_run(const char *app_id, const char *ev, const char *r, const char *line); /* -> KEY_SKAIAPP_RUN (0x29) JS app install/run/log report */
