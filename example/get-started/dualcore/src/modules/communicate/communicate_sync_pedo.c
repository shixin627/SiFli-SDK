...
static bool WristBandPedoDataRestore(uint8_t *buf, uint16_t len)
{
    ...
    for (uint8_t i = 0; i < len; i = i + 4)
    {
        if (flash_auto_read_locked((RtkWristbandSys.sport_address.p_read_addr + i) | 0x4000000,
                                   &data) == false)
        {
...
{
    APP_PRINT_INFO0("send sport data!");
    ...
    psendbuf[4] = PEDO_HEAD_LENGTH + PEDO_ITEM_LENGTH; /* length low */
    while (WristBandPedoDataRestore(buffer, PEDO_ITEM_LENGTH + PEDO_HEAD_LENGTH) == true)
    {
        memcpy(psendbuf + 5, buffer, PEDO_HEAD_LENGTH + PEDO_ITEM_LENGTH);
        if (wristband_send(psendbuf, 5 + PEDO_HEAD_LENGTH + PEDO_ITEM_LENGTH) == false)
        {
    ...
    if (RtkWristbandSys.gPedoData.quarter_steps != 0)
    {
        ...
        psendbuf[5] = mSportHead.Date.data >> 8;
        psendbuf[6] = mSportHead.Date.data;
        psendbuf[7] = 0;
        psendbuf[8] = 1;

        psendbuf[9] = mSportsData.data >> 56;
        psendbuf[10] = mSportsData.data >> 48;
        psendbuf[11] = mSportsData.data >> 40;
        psendbuf[12] = mSportsData.data >> 32;
        psendbuf[13] = mSportsData.data >> 24;
        psendbuf[14] = mSportsData.data >> 16;
        psendbuf[15] = mSportsData.data >> 8;
        psendbuf[16] = mSportsData.data;

        if (wristband_send(psendbuf, 5 + 12) == false)
        {
...
{
    APP_PRINT_INFO0("minute sport handler");
