...
{
    if (lv_disp_get_rotation(NULL) != LV_DISP_ROT_90 && lv_disp_get_rotation(NULL) != LV_DISP_ROT_270)
    {
...
static void app_clock_main_status_bar_event_cb(lv_event_t *event)
{
    ...
    //     LOG_D("app_clock_main_status_bar_event_cb %p, got event %s", obj, lv_event_to_name(event->code));
    switch (event->code)
    {
    case LV_EVENT_SCROLL:
    {
        ...
        lv_coord_t scroll_second_y = (466 - lv_obj_get_scroll_y(obj)) * bg_opa_2 / 466;
        lv_coord_t scroll_second_x = (466 - lv_obj_get_scroll_x(obj)) * bg_opa_2 / 466;
        if (((abs(scroll_y) < (bg_opa + 1)) && (scroll_y != 0)) || ((abs(scroll_x) < (bg_opa + 1)) && (scroll_x != 0))) // 230
        {
            if (scroll_y == 0)
            {
                shady_transparency = abs(scroll_x);
                lv_obj_set_style_bg_opa(myLancher[app_index_message].pagetileview, shady_transparency, 0);
            }
            else
            {
        ...
        // if (shady_transparency < (bg_opa_2 + 1) && shady_transparency > 0)
        // {
        //     lv_obj_set_style_img_opa(get_app_list_bluetooth_disconnection(), bg_opa_2 - shady_transparency, 0);
        // }
        // else if (shady_transparency >= bg_opa_2)
        // {
        //     lv_obj_set_style_img_opa(get_app_list_bluetooth_disconnection(), LV_OPA_COVER, 0);
        // }

        lv_coord_t scroll_third_y = (466 - lv_obj_get_scroll_y(obj)) * bg_opa_3 / 466;
    ...
    case LV_EVENT_VALUE_CHANGED:
    {
        ...
        if (active_pos == 1 && !lv_obj_has_flag(app_clock_main_status_bar, LV_OBJ_FLAG_HIDDEN) && lv_obj_get_scroll_x(myLancher[app_index_message].pagetileview) == 466)
        {
            lv_obj_add_flag(app_clock_main_status_bar, LV_OBJ_FLAG_HIDDEN);
        }
        ...
        if (1 == active_pos)
        {
            ...
            lv_obj_set_style_bg_opa(myLancher[app_index_message].pagetileview, shady_transparency, 0);
            lv_obj_add_flag(myLancher[app_index_message].pagetileview, LV_OBJ_FLAG_HIDDEN);
            ...
            lv_obj_set_style_bg_opa(get_app_list_battery_bg(), LV_OPA_TRANSP, 0);
        }
        else
        {
            lvgl_msg_t msg;
            ...
            lv_obj_clear_flag(myLancher[app_index_message].pagetileview, LV_OBJ_FLAG_HIDDEN);
            if (active_pos == MAIN_PAGE_TYPE_LEFT) // || active_pos == MAIN_PAGE_TYPE_UP
            {
                if (lv_obj_is_valid(get_app_list_time_bg()))
                    lv_obj_set_style_bg_opa(get_app_list_time_bg(), LV_OPA_80, 0);
            ...
            lv_obj_set_style_bg_opa(myLancher[app_index_message].pagetileview, LV_OPA_80, 0);
            if (active_pos == 0)
            {
...
static void app_clock_main_status_bar_down_event_cb(lv_event_t *event)
{
    ...
    case LV_EVENT_RELEASED:
    {
        ...
        if (lv_obj_get_scroll_x(myLancher[app_index_message].pagetileview) == 466 && lv_obj_get_scroll_y(myLancher[app_index_message].pagetileview) == 466)
            lv_obj_add_flag(app_clock_main_status_bar, LV_OBJ_FLAG_HIDDEN);
        break;
    }
    default:
...
void animate_to_notification_center(void)
{
    ...
    lv_obj_clear_flag(myLancher[app_index_message].pagetileview, LV_OBJ_FLAG_HIDDEN);
    lv_obj_set_tile_id(myLancher[app_index_message].pagetileview, 0, 1, LV_ANIM_ON);
...
void animate_to_home_from_notification_center(void)
{
    if (lv_obj_is_valid(myLancher[app_index_message].pagetileview))
        lv_obj_set_tile_id(myLancher[app_index_message].pagetileview, 1, 1, LV_ANIM_ON);
}
...
void animate_to_message_list(void)
{
    ...
    if (lv_obj_is_valid(myLancher[app_index_message].pagetileview))
    {
        set_need_open_gesture_control(true);
        lv_obj_clear_flag(myLancher[app_index_message].pagetileview, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_tile_id(myLancher[app_index_message].pagetileview, 1, 0, LV_ANIM_ON);
    }
    // set_scroll_anim_time(false);
}
...
void animate_to_ai_page(void)
{
    ...
    {
        lv_obj_clear_flag(myLancher[app_index_ai_interface].pagetileview, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_tile_id(myLancher[app_index_ai_interface].pagetileview, 1, 0, LV_ANIM_ON);
    }
    // set_scroll_anim_time(false);
}
...
void animate_to_home_from_ai_page(void)
{
    ...
    if (lv_obj_is_valid(myLancher[app_index_ai_interface].pagetileview))
        lv_obj_set_tile_id(myLancher[app_index_ai_interface].pagetileview, 0, 0, LV_ANIM_ON);
}
...
static void gesture_collect_btn_event_cb(lv_event_t *e)
{
    ...
    if (imu_data_collection_error)
    {
        ...
        lv_obj_set_style_bg_color(gesture_collect_btn, lv_color_hex(0xFFFFFF), 0);
...
static void gesture_select_icon_event_cb(lv_event_t *e)
{
    if (gesture_tap_collection)
    {
        ...
        lv_obj_set_style_bg_color(gesture_select_icon, lv_color_hex(0xFFFFFF), 0);
...
{
    if (!lv_obj_is_valid(brightness_bar) && (MSG_SERVICE_SUBSCRIBE_RSP != arg->msg_id))
    {
        return 0;
    }
...
static void bar_event_cb(lv_event_t *e)
{
    ...
    else if (code == LV_EVENT_PRESSED)
    {
        ...
        lv_obj_clear_flag(myLancher[app_index_message].pagetileview, LV_OBJ_FLAG_SCROLLABLE);
    }
    else if (code == LV_EVENT_RELEASED)
    {
        LOG_D("LV_EVENT_RELEASED_Brightness Bar");
        lv_obj_add_flag(myLancher[app_index_message].pagetileview, LV_OBJ_FLAG_SCROLLABLE);
    }
}

extern void build_media_contorll_widget(app_media_t *p_app_media, lv_obj_t *parent);
...
static lv_obj_t *control_center_layout_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_bg_color(control_center_window, LV_COLOR_WHITE, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(control_center_window, LV_OPA_0, LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_t *bar = lv_bar_create(control_center_window); // Create a progress bar
    ...
    // setting icon
    // --- Mouse mode button
    lv_obj_t *calculator_btn = common_image_button(control_center_bottom, CALCULATOR_ICON, 100, 100, calculator_btn_event_cb);
    ...
    lv_obj_t *recorder_btn = common_image_button(control_center_bottom, &micro_icon, 100, 100, recorder_btn_event_cb);
    ...
    lv_obj_t *flishlight_btn = common_image_button(control_center_bottom, FLISHLIGHT_ICON, 100, 100, flishlight_icon_event_cb);
    ...
    lv_obj_t *setting_icon = common_image_button(control_center_bottom, IMG_SETTINGS, 100, 100, setting_icon_event_cb);
    ...
    // --- QRcode button
    lv_obj_t *qrcode_btn = common_image_button(control_center_bottom, &icon_qrcode, 100, 100, qrcode_btn_event_cb);
    ...
    dnd_mode_btn = common_image_button(control_center_bottom, &icon_dnd_mode, 100, 100, dnd_mode_btn_event_cb);
    ...
    lv_obj_t *mouse_btn = common_image_button(control_center_bottom, &mouse_mode_icon, 100, 100, mouse_mode_icon_event_cb);
    ...
    // -------------- find phone button
    lv_obj_t *find_phone_btn = common_image_button(control_center_bottom, FIND_PHONE, 100, 100, find_phone_btn_event_cb);
...
#if !kReleaseMode
    // Gesture test app
    lv_obj_t *gesture_test_btn = common_image_button(control_center_bottom, IMG_LOGO, 100, 100, gesture_test_btn_event_cb);
...
void refresh_ble_mode_btn(void)
{
    ...
    if (get_bluetooth_broadcasting_status() || SkaiWatchSys.gap_conn_state == GAP_CONN_STATE_CONNECTED)
    {
    ...
    for (int i = 0; i < 5; i++)
    {
        if (open)
        {
            if (lv_obj_is_valid(pages[i]))
                lv_obj_set_style_bg_opa(pages[i], LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
        }
        else
        {
            if (lv_obj_is_valid(pages[i]))
...
void app_clock_main_status_bar_init(lv_obj_t *par)
{
    ...
    lv_obj_set_style_bg_opa(status_bar_bg_main, LV_OPA_0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(status_bar_bg_main, LV_COLOR_WHITE, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        lv_obj_set_style_bg_opa(status_bar_area, bar_opa, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_add_flag(status_bar_area, LV_OBJ_FLAG_EVENT_BUBBLE);

        if (STATUS_BAR_AREA_LEFT == i)
        {
            lv_obj_set_size(status_bar_area, (LV_HOR_RES_MAX >> 3), LV_VER_RES_MAX);
            lv_obj_set_user_data(status_bar_area, (void *)STATUS_BAR_AREA_LEFT);
            lv_obj_add_event_cb(status_bar_area, notification_status_bar_cb, LV_EVENT_ALL, NULL);
            lv_obj_align(status_bar_area, LV_ALIGN_LEFT_MID, 0, 0);
            status_bar_area_left = status_bar_area;
        }
        else if (STATUS_BAR_AREA_UP == i)
        {
            lv_obj_set_size(status_bar_area, LV_HOR_RES_MAX, (LV_VER_RES_MAX >> 3));
    ...
    lv_obj_set_style_bg_color(app_clock_main_status_bar, LV_COLOR_BLACK, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(app_clock_main_status_bar, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
    // 0: down, 1: home, 2: up, 3: left, 4: right
    for (i = 0; i < 4; i++)
    {
        if (i == MAIN_PAGE_TYPE_HOME)
        {
            pages[i] = lv_tileview_add_tile(app_clock_main_status_bar, 1, i, LV_DIR_ALL);
            app_clock_main_status_bar_down = pages[i];
            lv_obj_set_style_bg_color(pages[i], LV_COLOR_RED, LV_PART_MAIN | LV_STATE_DEFAULT);
            if (!test_mode)
            {
                lv_obj_set_style_bg_opa(pages[i], LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
            }
            ...
            lv_obj_add_event_cb(pages[i], app_clock_main_status_bar_down_event_cb, LV_EVENT_ALL, NULL);
        ...
        else
        {
            ...
            {
                pages[i] = lv_tileview_add_tile(app_clock_main_status_bar, 1, 2, LV_DIR_VER);
            }
            else if (i == MAIN_PAGE_TYPE_LEFT)
            {
                pages[i] = lv_tileview_add_tile(app_clock_main_status_bar, 0, 1, LV_DIR_HOR);
            }
            else if (i == MAIN_PAGE_TYPE_UP)
            {
                pages[i] = lv_tileview_add_tile(app_clock_main_status_bar, 1, 0, LV_DIR_VER);
            }
            else if (i == MAIN_PAGE_TYPE_RIGHT)
            {
                pages[i] = lv_tileview_add_tile(app_clock_main_status_bar, 2, 1, LV_DIR_VER);
            }
            if (!test_mode)
            {
                lv_obj_set_style_bg_opa(pages[i], LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
            ...
            if (i == 0)
            {
                lv_obj_set_style_bg_color(pages[i], LV_COLOR_YELLOW, LV_PART_MAIN | LV_STATE_DEFAULT);
            }
            else if (i == 2)
            {
                lv_obj_set_style_bg_color(pages[i], LV_COLOR_GREEN, LV_PART_MAIN | LV_STATE_DEFAULT);
            }
            else if (i == 3)
            {
                lv_obj_set_style_bg_color(pages[i], LV_COLOR_WHITE, LV_PART_MAIN | LV_STATE_DEFAULT);
            }
            else if (i == 4)
            {
                lv_obj_set_style_bg_color(pages[i], LV_COLOR_BLUE, LV_PART_MAIN | LV_STATE_DEFAULT);
            }
            lv_obj_set_size(pages[i], LV_HOR_RES_MAX, LV_VER_RES_MAX);
            lv_obj_set_scrollbar_mode(pages[i], LV_SCROLLBAR_MODE_OFF);
            // lv_obj_add_event_cb(pages[i], app_clock_main_tileview_event, LV_EVENT_ALL, NULL);
        }
    }
    // pages[2] = lv_tileview_add_tile(app_clock_main_status_bar, 0, 1, LV_DIR_RIGHT);
    // if (TILEVIEW_HIDDEN_CLOSE)
    // {
    //     lv_obj_set_style_bg_opa(pages[2], LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
    // }
    // else
    // {
    //     lv_obj_set_style_bg_opa(pages[2], LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
    // }
    // lv_obj_set_style_bg_color(pages[2], LV_COLOR_BLACK, LV_PART_MAIN | LV_STATE_DEFAULT);
    // lv_obj_set_size(pages[2], LV_HOR_RES_MAX, LV_VER_RES_MAX);
    ...
    lv_obj_add_event_cb(app_clock_main_status_bar, app_clock_main_status_bar_event_cb, LV_EVENT_ALL, NULL);
...
void app_clock_ai_status_bar_init(lv_obj_t *par)
{
    ...
    lv_obj_set_style_bg_opa(status_bar_bg_ai, LV_OPA_0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(status_bar_bg_ai, LV_COLOR_WHITE, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_size(status_bar_area, (LV_HOR_RES_MAX >> 4), (LV_VER_RES_MAX >> 2));
    lv_obj_set_scrollbar_mode(status_bar_area, LV_SCROLLBAR_MODE_OFF);
    lv_obj_clear_flag(status_bar_area, LV_OBJ_FLAG_PRESS_LOCK); // Allow press event to tileview
    lv_obj_set_style_bg_opa(status_bar_area, bar_opa, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_bg_color(app_clock_ai_status_bar, LV_COLOR_BLACK, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(app_clock_ai_status_bar, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);

    // Create a single tile for AI status bar
    for (i = 0; i < 2; i++)
    {
        pages[i] = lv_tileview_add_tile(app_clock_ai_status_bar, i, 0, LV_DIR_HOR);
        if (i == 0)
        {
            lv_obj_add_event_cb(pages[i], app_clock_main_ai_status_bar_event_cb, LV_EVENT_ALL, NULL);
            lv_obj_set_style_bg_color(pages[i], LV_COLOR_BLACK, LV_PART_MAIN | LV_STATE_DEFAULT);
        }
        else
        {
            lv_obj_set_style_bg_color(pages[i], LV_COLOR_WHITE, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_add_event_cb(app_clock_ai_status_bar, app_clock_ai_status_bar_event_cb, LV_EVENT_ALL, NULL);
