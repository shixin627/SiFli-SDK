...
#define SCREEN_HOR_RES_MAX 466
#define SCREEN_VER_RES_MAX 466
...
const lv_obj_class_t lv_arc_scrollbar_class = {
    ...
    .width_def = SCREEN_HOR_RES_MAX,  // LV_HOR_RES,
    .height_def = SCREEN_VER_RES_MAX, // LV_VER_RES,
...
static void lv_arc_scrollbar_event(const lv_obj_class_t *class_p, lv_event_t *evt)
{
    ...
    if (evt->code == LV_EVENT_DRAW_MAIN_BEGIN)
    {
        if (arc->ind_height < 0)
        {
            ...
            arc->ind_height = 2 * Y_MAX * LV_VER_RES / arc->total_height;
        }
    }
