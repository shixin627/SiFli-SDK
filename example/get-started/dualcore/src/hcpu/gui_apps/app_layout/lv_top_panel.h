 ...
 *
 *  滑鼠模式圖層仍由本檔管理（R47 2026-09-05）：
 *    - 進入 = 錶盤往上拉 settle 在下方停車位 (1,2) → lv_top_panel_mouse_enter()
 *    - 退出 = 滑鼠頁頂部下拉 → settle 回錶盤
...
/** 進入滑鼠模式（R47：錶盤**往上拉** settle 在下方停車位 (1,2) 時由 clock
    呼叫）。控制目標（上次控制的那台）由呼叫端先選好。 */
void lv_top_panel_mouse_enter(void);

/** 退出滑鼠模式（拆圖層 + 還原邊緣 zone）。落回錶盤 —— 入口是滑鼠頁頂部下拉
    的 clock_main_mouse_pulldown_reveal()。 */
void lv_top_panel_mouse_exit(void);

/** 設備清單有變（registry 更新）→ 轉發給錶盤（R47 之後右側是固定的控制中心 +
    App List，這條已是 no-op，留著給既有呼叫端）。 */
void lv_top_panel_refresh_devices(void);

/** 舊 pager 時代的入口，現為 no-op（通知列表中央卡拖曳仍呼叫）。 */
void lv_top_panel_set_hor_enabled(bool enabled);

/** 下拉退出時，滑鼠圖層整層跟著手指往下位移（0 = 原位，466 = 完全推出畫面下方）。
    非滑鼠模式 no-op。 */
