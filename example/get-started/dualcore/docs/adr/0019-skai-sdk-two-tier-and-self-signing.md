...
- **Status**: accepted (2026-08-03), Phase 0 landed; Phase 3 on the watch 2026-09-19 (decision 15)
...
### 15. On the watch (2026-09-19): space, transport, and what the AI author needs

The runtime left the simulator. What it took, all measured on the watch build:

- **Space came from images, not the partition.** With QuickJS in, `main.bin` was
  256 KB over `0x280000`. Growing `main` would break OTA for watches in the field
  (the FS starts right after it), so the 28 notification / service logos and the
  earth watch-face background (~370 KB) moved to `/assets/icons/*.bin` on the FS
  (`UI_FS_ICON`, `ui_fs_img_or()` falls back to the built-in generic glyph when a
  file is missing). `project/hcpu/gen_fs_icons.py` emits the files from the resource
  build into `project/jsroot/` (new watches) and the phone's assets; the phone copies
  them down on every connection (compare-then-skip). Result: 2,587,440 B with the
  runtime in, ~34 KB free.
- **Transport = the existing 0x13 push, carrying an envelope.** `"SKJS" | flags |
  mlen u32 | manifest | payload` — the manifest and payload reach `skai_pkg_install()`
  byte for byte, so the signature is checked over exactly what was sent. flags bit0
  opens the app. Up to 20 × 3 KB frames, reassembled in PSRAM. The install runs
  on a short-lived worker thread, never the BLE parse thread: the first
  blocklist KVDB open after a boot scans flash for seconds, and blocking the BLE
  host that long made the LCPU give up and reset the watch (seen on the first
  phone push after every reboot). Both KVDB stores are also pre-warmed 20 s after
  boot on a low-priority thread.
- **The author is usually an AI on the phone, so failure has to talk.** New uplink
  0x29 `skaiappRun {"id","ev":install|start|log|stop,"r","line"}` carries the
  install verdict, the first-run result and every app-log line (rate-limited to
  8/s). The phone's `write_watch_program` tool returns these to the model so it can
  fix and reinstall in the same conversation.
- **Standard JS globals, still inside the sandbox:** `setTimeout/setInterval/clear*`
  (lv_timer on the LVGL thread, 16 max, ≥50 ms intervals, each firing re-arms the
  watchdog like a click) and `console.log/info/warn/error` → app log.
- **Widgets an app actually needs:** `ui.list` (scrolling group), `ui.item`,
  `ui.checkbox`, `ui.switch`, `ui.slider`, `ui.value/set_value/set_range`,
  `ui.set_visible`, `ui.remove`, `ui.scroll_to`; slots 48 → 160. A deleted widget
  frees its slot via LV_EVENT_DELETE, so removing a list cannot leave dangling ids.
  The checkbox tick is drawn with two lines — a symbol font for one glyph was 16 KB.
- **JS source 16 → 48 KB, moved to PSRAM** (as the note in skai_pkg.h asked), and
  read in place by the host rather than copied.

Verified on the bench watch, including over BLE from the phone (Android Keystore
signing, 0x29 reports back into the tool result): a phone-signed package (Android-Keystore-shaped ES256,
generated by the phone's unit test) installs through the watch's verifier, draws a
10-row CJK checklist, reacts to taps, runs timers, and a runaway loop / throwing
handler are stopped and reported without resetting the watch.
