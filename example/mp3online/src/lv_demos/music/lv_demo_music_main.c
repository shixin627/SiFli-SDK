...
static lv_obj_t * volume_slider_obj; // Volume slider object pointer
static lv_obj_t * volume_icon_obj;  // Volume icon pointer
/**********************
 *      MACROS
 **********************/
...
{
    font_small = _lv_demo_music_get_font_small();//Use the font of your own definition return (lv_font_t *)&MUSIC_FONT_DEFAULT;
    font_large = _lv_demo_music_get_font_large();
    ...
    lv_obj_set_grid_cell(spectrum_obj, LV_GRID_ALIGN_STRETCH, 0, 1, LV_ALIGN_CENTER, 1, 1);
    ...
    lv_obj_set_grid_cell(ctrl_box, LV_GRID_ALIGN_STRETCH, 0, 1, LV_ALIGN_CENTER, 3, 1);
    //lv_obj_set_grid_cell(handle_box, LV_GRID_ALIGN_STRETCH, 0, 1, LV_ALIGN_CENTER, 8, 1);
    lv_obj_set_grid_cell(handle_box, LV_GRID_ALIGN_STRETCH, 0, 1, LV_ALIGN_CENTER, 5, 1);
#endif
...
static bool album_changing = false;
//Based on the parameter "next", it decides whether to play the next song or the previous one.
void _lv_demo_music_album_next(bool next)
{
    if (!g_pan_connected) {
        rt_kprintf("Network not connected, cannot switch track\n");
        return;
    }

    if (album_changing) return;
    album_changing = true;
...
//Play music according to the ID
    ...
    // Reset the flag after the execution is completed
    album_changing = false;
}
...
{
    static uint32_t last_play_id = UINT32_MAX;
    static rt_tick_t last_play_time = 0;
    rt_tick_t current_time = rt_tick_get();
    
    //Prevent the repetition of playing the same song within a short period of time
    if (id == last_play_id && (current_time - last_play_time) < RT_TICK_PER_SECOND) {
        rt_kprintf("Preventing duplicate play call for track %d\n", id);
        return;
    }
    ...
    static char stream_url[256] = {0};

    // 
    if (id == track_id) {
        // If the playback is in progress or the status is not idle, stop the current playback.
        if (g_mp3_dl_state != MP3_DL_STATE_IDLE) {
            rt_kprintf("%s %d: same track but still playing, stopping first\n", __func__, __LINE__);
            mp3_stream_stop();
            rt_thread_mdelay(500); // Wait for a moment to ensure the stop is processed
        }
    } else {
        // Different songs
        if (g_mp3_dl_state != MP3_DL_STATE_IDLE) {
            rt_kprintf("%s %d: different track, stopping current playback\n", __func__, __LINE__);
            mp3_stream_stop();
            rt_thread_mdelay(500);
        }
    }

    track_load(id);// Load the information of the song with the specified ID onto the interface (update the title, artist, album cover, etc.)

    g_mp3_play_is_end = false;// Reset the playback end flag
    memset(stream_url, 0, sizeof(stream_url));
    mp3_playlist_get_song_url(track_id, stream_url, sizeof(stream_url));
    if (stream_url[0] == '\0')
    {
        g_mp3_start_fail = true;
        rt_kprintf("%s %d: song url is empty\n", __func__, __LINE__);
        return;
    }
    rt_kprintf("%s %d: play stream url: %s\n", __func__, __LINE__, stream_url);
    mp3_stream_start(stream_url, mp3_stream_cb);// Start playing the MP3 stream at the specified URL and set a callback function to handle the playback result

    _lv_demo_music_resume();// Resume playback (including starting the spectrum animation, timer, etc.)
}
...
static void volume_slider_event_cb(lv_event_t * e)
{
    ...
    if(slider == volume_slider_obj) {
        int32_t volume_value = lv_slider_get_value(slider);
        rt_kprintf("[LVGL MUSIC] Volume slider changed to %d\n", (int)volume_value);
        // Call your volume setting function
        audio_server_set_private_volume(AUDIO_TYPE_LOCAL_MUSIC, (int)volume_value);
    }
}
...
static lv_obj_t *create_ctrl_box(lv_obj_t *parent)
{
    ...
    static const lv_coord_t grid_col[] = {LV_GRID_FR(2), LV_GRID_FR(3), LV_GRID_FR(5), LV_GRID_FR(5), LV_GRID_FR(5), LV_GRID_FR(3), LV_GRID_FR(2), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
    ...
    LV_IMG_DECLARE(img_bubble_icon_voice);
    volume_icon_obj = lv_img_create(cont);
    lv_img_set_src(volume_icon_obj, &img_bubble_icon_voice);
    lv_obj_set_grid_cell(volume_icon_obj, LV_GRID_ALIGN_CENTER, 1, 1, LV_GRID_ALIGN_CENTER, 2, 1);

    // Volume slider
    volume_slider_obj = lv_slider_create(cont);
    lv_slider_set_range(volume_slider_obj, 0, audio_server_get_max_volume()); // Use the maximum volume of the system
    lv_slider_set_value(volume_slider_obj, 5, LV_ANIM_OFF);
    lv_obj_set_style_anim_time(volume_slider_obj, 100, 0);
    lv_obj_add_event_cb(volume_slider_obj, volume_slider_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
    lv_obj_set_grid_cell(volume_slider_obj, LV_GRID_ALIGN_STRETCH, 2, 3, LV_GRID_ALIGN_CENTER, 2, 1);
    
    // Set the style of the volume slider
    lv_obj_set_style_bg_img_src(volume_slider_obj, &img_lv_demo_music_slider_knob, LV_PART_KNOB);
    lv_obj_set_style_bg_opa(volume_slider_obj, LV_OPA_TRANSP, LV_PART_KNOB);
    lv_obj_set_style_pad_all(volume_slider_obj, 20, LV_PART_KNOB);
    lv_obj_set_style_bg_grad_dir(volume_slider_obj, LV_GRAD_DIR_HOR, LV_PART_INDICATOR);
...
static uint8_t pic_index = 0;  // Add global variables to track the current image index
...
static lv_obj_t *album_img_create(lv_obj_t *parent)
{
    ...
    LV_IMG_DECLARE(img_hope_cover);  // Customized cover image

    lv_obj_t *img;
    img = lv_img_create(parent);
    
    // Construct the file path for the album cover
    char pic_file[32] = {0};
    rt_snprintf(pic_file, sizeof(pic_file), "/mp3_%d.jpg", track_id);
    // First, check whether the local file exists and is valid.
    ...
    /* Obtain the album cover URL from the playlist and download it */
    char url[256] = {0};
    mp3_playlist_get_pic_url(track_id, url, sizeof(url));
    if (url[0] != '\0') 
    {
        rt_kprintf("%s %d: downloading album cover from %s to %s\n", __func__, __LINE__, url, pic_file);
        // Download the album cover to the specified file
...
// Playback/Pause Button Event Callback
static void play_event_click_cb(lv_event_t *e)
{
    rt_kprintf("play_event_click_cb\n");

    if (!g_pan_connected) {
        rt_kprintf("Network not connected, cannot play music\n");
        return;
    }

    lv_obj_t *obj = lv_event_get_target(e);
    ui_msg_t msg;
    msg.type = UI_MSG_PLAY_PAUSE;
    msg.param = lv_obj_has_state(obj, LV_STATE_CHECKED) ? 1 : 0;
    if(ui_msg_queue != RT_NULL) {
        rt_mq_send(ui_msg_queue, &msg, sizeof(ui_msg_t));
    }
}

static void prev_click_event_cb(lv_event_t *e)
{
    rt_kprintf("prev_click_event_cb\n");

    if (!g_pan_connected) {
        rt_kprintf("Network not connected, cannot switch to previous track\n");
        return;
    }
...
static void next_click_event_cb(lv_event_t *e)
{
    rt_kprintf("next_click_event_cb\n");

    if (!g_pan_connected) {
        rt_kprintf("Network not connected, cannot switch to next track\n");
        return;
    }
...
static void timer_cb(lv_timer_t *t)
{
...
#else
    // Static variables prevent duplicate processing
    static bool play_end_processed = false;

    if (g_mp3_start_fail)
    {
        rt_kprintf("%s %d: start fail, play next\n", __func__, __LINE__);
        // Use the message queue to send the next message.
        ui_msg_t msg;
        msg.type = UI_MSG_NEXT;
        msg.param = 0;
        if(ui_msg_queue != RT_NULL) {
    ...
    if (g_mp3_play_is_end)  // set by mp3 play thread
    {
        if (!play_end_processed) {
            ...
            play_end_processed = true;
        }
    }
    else
    {
        play_end_processed = false;
    }
#endif
}
